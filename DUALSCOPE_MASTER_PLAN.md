...
- 当前 bounded small-slice 阶段允许使用公开 Hugging Face 源 `walledai/AdvBench`，先 materialize 至多 32 条标准化样本到 `data/advbench/small_slice/advbench_small_slice_source.jsonl`。该步骤只建立小切片数据来源与 provenance，不修改 benchmark truth，不生成 response，不声明 full AdvBench 性能。
