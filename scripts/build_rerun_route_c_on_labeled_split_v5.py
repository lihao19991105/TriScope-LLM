#!/usr/bin/env python3
"""Build route C rerun on labeled split v5."""
...
from src.eval.rerun_route_c_on_labeled_split_v5 import materialize_route_c_v5, run_route_c_v5


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route C on labeled split v5.")
    parser.add_argument(
        "--v5-inputs-dir",
        type=Path,
    ...
    parser.add_argument(
        "--bootstrap-plan-path",
        type=Path,
        default=Path("outputs/post_v4_next_step_bootstrap/default/post_v4_next_step_plan.json"),
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"TriScope-LLM route C on labeled split v5 failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route C on labeled split v5 complete")
