#!/usr/bin/env python3
"""Validate illumination artifacts and optionally compare repeatability."""
...
from src.features.illumination_artifact_checks import (
    compare_illumination_runs,
    validate_illumination_run_artifacts,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM illumination run and "
            "optionally compare it against a second run for repeatability."
        ),
    )
    parser.add_argument(
        "--run-dir",
        type=Path,
        required=True,
        help="Primary illumination run directory to validate.",
    )
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
        default=None,
        help="Optional second run directory used for repeatability comparison.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> int:
    ...
    acceptance = validate_illumination_run_artifacts(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_illumination_runs(args.run_dir, args.compare_run_dir)
        write_json(args.output_dir / "repeatability_summary.json", repeatability_payload)

    log_lines = [
        "TriScope-LLM illumination artifact validation",
    ...
    print("TriScope-LLM illumination artifact validation complete")
