#!/usr/bin/env python3
"""Build route C rerun on labeled split v3."""
...
from src.eval.rerun_route_c_on_labeled_split_v3 import materialize_route_c_v3, run_route_c_v3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route C on labeled split v3.")
    parser.add_argument(
        "--v3-inputs-dir",
        type=Path,
    ...
    parser.add_argument(
        "--bootstrap-plan-path",
        type=Path,
        default=Path("outputs/post_v2_next_step_bootstrap/default/post_v2_next_step_plan.json"),
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"TriScope-LLM route C on labeled split v3 failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route C on labeled split v3 complete")
