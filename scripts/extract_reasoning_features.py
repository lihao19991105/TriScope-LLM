#!/usr/bin/env python3
"""Extract structured reasoning features from raw probe artifacts."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Extract sample-level and run-level reasoning features from a raw_results.jsonl "
            "artifact produced by the TriScope-LLM reasoning probe."
        ),
    )
    parser.add_argument(
        "--raw-results",
        type=Path,
        required=True,
        help="Path to the reasoning raw_results.jsonl artifact.",
    )
    parser.add_argument(
        "--summary-json",
        type=Path,
        default=None,
        help="Optional path to the reasoning summary.json artifact.",
    )
    parser.add_argument(
        "--config-snapshot",
        type=Path,
        default=None,
        help="Optional path to the reasoning config_snapshot.json artifact.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> int:
    ...
    try:
        ...
        print(f"TriScope-LLM reasoning feature extraction failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    summary = result["feature_summary"]
    output_paths = result["output_paths"]
    print("TriScope-LLM reasoning feature extraction complete")
    print(f"Run ID: {summary['run_id']}")
    print(f"Samples: {summary['num_samples']}")
    print(f"Answer changed rate: {summary['answer_changed_rate']:.4f}")
    print(f"Target behavior flip rate: {summary['target_behavior_flip_rate']:.4f}")
