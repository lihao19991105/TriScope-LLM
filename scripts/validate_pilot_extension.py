#!/usr/bin/env python3
"""Validate pilot extension artifacts and optionally compare repeatability."""
...
from src.eval.pilot_extension_checks import compare_pilot_extension_runs, validate_pilot_extension


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM pilot extension run, "
            "and optionally compare it against a second extension run for repeatability."
        ),
    )
    parser.add_argument("--run-dir", type=Path, required=True, help="Primary pilot extension run directory.")
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
        default=None,
        help="Optional second pilot extension run directory used for repeatability comparison.",
    )
...
def main() -> int:
    ...
    acceptance = validate_pilot_extension(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_pilot_extension_runs(
    ...
    log_lines = [
        "TriScope-LLM pilot extension validation",
    ...
    print("TriScope-LLM pilot extension validation complete")
