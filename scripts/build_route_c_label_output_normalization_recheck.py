#!/usr/bin/env python3
"""Build route_c label output normalization controlled recheck artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build route_c label output normalization controlled recheck artifacts.")
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    from src.eval.route_c_label_output_normalization_recheck import (
        build_route_c_label_output_normalization_recheck,
    )

    try:
        result = build_route_c_label_output_normalization_recheck(
        ...
        print(f"TriScope-LLM route_c label output normalization recheck failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route_c label output normalization recheck complete")
