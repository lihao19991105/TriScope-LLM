#!/usr/bin/env python3
"""Validate cross-pilot reporting artifacts and optionally compare repeatability."""
...
from src.eval.cross_pilot_report_checks import compare_cross_pilot_reports, validate_cross_pilot_report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM cross-pilot report, "
            "and optionally compare it against a second run for repeatability."
        ),
    )
    parser.add_argument("--run-dir", type=Path, required=True, help="Primary cross-pilot report directory.")
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
        default=None,
        help="Optional second cross-pilot report directory used for repeatability comparison.",
    )
...
def main() -> int:
    ...
    acceptance = validate_cross_pilot_report(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_cross_pilot_reports(
    ...
    log_lines = [
        "TriScope-LLM cross-pilot report validation",
    ...
    print("TriScope-LLM cross-pilot report validation complete")
