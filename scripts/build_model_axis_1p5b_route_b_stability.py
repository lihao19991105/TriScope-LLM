#!/usr/bin/env python3
"""Build route_b stability confirmation artifacts for model-axis 1.5B."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build model-axis 1.5B route_b stability confirmation artifacts.")
    parser.add_argument(
        "--stable-reference-run-summary-path",
        type=Path,
        default=Path("outputs/model_axis_1p5b_route_b_stable_execution/default/route_b_stable_execution_run_summary.json"),
    )
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    from src.eval.model_axis_1p5b_route_b_stability import build_model_axis_1p5b_route_b_stability

    try:
        result = build_model_axis_1p5b_route_b_stability(
            stable_reference_run_summary_path=args.stable_reference_run_summary_path,
        ...
        print(f"TriScope-LLM model-axis 1.5B route_b stability failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM model-axis 1.5B route_b stability complete")
    print(f"Summary: {result['output_paths']['summary']}")
    print(f"Results: {result['output_paths']['results']}")
    print(f"Comparison: {result['output_paths']['comparison_csv']}")
    return 0
