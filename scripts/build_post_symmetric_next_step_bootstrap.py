#!/usr/bin/env python3
"""Build the post-symmetric next-step bootstrap."""
...
from src.eval.post_symmetric_next_step_bootstrap import build_post_symmetric_next_step_bootstrap


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build the post-symmetric next-step bootstrap.")
    parser.add_argument(
        "--current-larger-inputs-dir",
        type=Path,
    ...
    parser.add_argument(
        "--recommendation-path",
        type=Path,
        default=Path("outputs/symmetric_larger_split_comparison/default/symmetric_rerun_next_step_recommendation.json"),
    )
...
def main() -> int:
    ...
    try:
        result = build_post_symmetric_next_step_bootstrap(
            current_larger_inputs_dir=args.current_larger_inputs_dir,
        ...
        print(f"TriScope-LLM post-symmetric next-step bootstrap failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM post-symmetric next-step bootstrap complete")
