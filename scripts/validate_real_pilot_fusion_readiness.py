#!/usr/bin/env python3
"""Validate real-pilot fusion readiness artifacts and optionally compare repeatability."""
...
from src.eval.real_pilot_fusion_checks import compare_real_pilot_fusion_readiness, validate_real_pilot_fusion_readiness


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate real-pilot fusion readiness artifacts and optionally compare repeatability.",
    )
...
def main() -> int:
    ...
    acceptance = validate_real_pilot_fusion_readiness(args.run_dir)
    acceptance_path = args.output_dir / "artifact_acceptance.json"
    write_json(acceptance_path, acceptance)

    repeatability = None
    if args.compare_run_dir is not None:
        repeatability = compare_real_pilot_fusion_readiness(args.run_dir, args.compare_run_dir)
        write_json(args.output_dir / "repeatability_summary.json", repeatability)

    log_lines = [
        "TriScope-LLM real-pilot fusion readiness validation",
    ...
    print("TriScope-LLM real-pilot fusion readiness validation complete")
