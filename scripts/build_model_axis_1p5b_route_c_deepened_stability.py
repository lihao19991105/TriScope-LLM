#!/usr/bin/env python3
"""Build deepened route_c stability artifacts for model-axis 1.5B."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build model-axis 1.5B deepened route_c stability artifacts.")
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    from src.eval.model_axis_1p5b_route_c_deepened_stability import build_model_axis_1p5b_route_c_deepened_stability

    try:
        result = build_model_axis_1p5b_route_c_deepened_stability(
        ...
        print(f"TriScope-LLM model-axis 1.5B route_c deepened stability failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM model-axis 1.5B route_c deepened stability complete")
