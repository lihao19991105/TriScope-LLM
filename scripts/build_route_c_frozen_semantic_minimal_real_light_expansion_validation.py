#!/usr/bin/env python3
"""CLI for stage-157A route_c frozen semantic minimal real light expansion validation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Stable real execution run directory used as real-light-expansion anchor.",
    )
    parser.add_argument(
        "--stage-154-dir",
        type=Path,
        default=Path("outputs/model_axis_1p5b_route_c_frozen_semantic_minimal_real_batched_continuous_regression/default"),
        help="Directory containing stage-154 artifacts.",
    )
    parser.add_argument(
        "--stage-155-dir",
        type=Path,
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory to write stage-157A artifacts.")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    from src.eval.route_c_frozen_semantic_minimal_real_light_expansion_validation import (
        build_route_c_frozen_semantic_minimal_real_light_expansion_validation,
    )

    try:
        result = build_route_c_frozen_semantic_minimal_real_light_expansion_validation(
        ...
        print(f"TriScope-LLM stage-157A minimal real light expansion validation failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("stage-157A route_c frozen semantic minimal real light expansion validation artifacts written to:")
