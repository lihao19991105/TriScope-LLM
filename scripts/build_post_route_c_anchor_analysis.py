#!/usr/bin/env python3
"""Build post anchor-aware route_c analysis artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build post anchor-aware route_c analysis artifacts.")
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    from src.eval.post_route_c_anchor_analysis import build_post_route_c_anchor_analysis

    try:
        result = build_post_route_c_anchor_analysis(
        ...
        print(f"TriScope-LLM post route_c anchor analysis failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM post route_c anchor analysis complete")
