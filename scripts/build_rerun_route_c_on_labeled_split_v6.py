#!/usr/bin/env python3
"""Build route C rerun on labeled split v6."""
...
from src.eval.rerun_route_c_on_labeled_split_v6 import materialize_route_c_v6, run_route_c_v6


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route C on labeled split v6.")
    parser.add_argument(
        "--v6-inputs-dir",
        type=Path,
    ...
    parser.add_argument(
        "--bootstrap-plan-path",
        type=Path,
        default=Path("outputs/post_v5_next_step_bootstrap/default/post_v5_next_step_plan.json"),
    )
    ...
    parser.add_argument(
        "--label-parser-instrumentation",
        action="store_true",
        help="Emit line-level parser decision details into label health rows.",
    )
    return parser
...
def main() -> int:
    ...
    try:
        materialized = materialize_route_c_v6(
            v6_inputs_dir=args.v6_inputs_dir,
            bootstrap_plan_path=args.bootstrap_plan_path,
            output_dir=args.output_dir,
        )
        executed = run_route_c_v6(
            ...
            label_parse_mode=args.label_parse_mode,
            label_normalization_mode=args.label_normalization_mode,
            label_parser_instrumentation=args.label_parser_instrumentation,
        )
        ...
        print(f"TriScope-LLM route C on labeled split v6 failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route C on labeled split v6 complete")
