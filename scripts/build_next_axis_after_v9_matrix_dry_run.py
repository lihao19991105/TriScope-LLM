#!/usr/bin/env python3
"""CLI for next-axis-after-v9 matrix dry-run."""
...
from src.eval.next_axis_after_v9_matrix_dry_run import build_next_axis_after_v9_matrix_dry_run


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build the refined-fusion-support-floor-stress-aware next-axis-after-v9 matrix dry-run."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/next_axis_after_v9_matrix_bootstrap/default/next_axis_after_v9_bootstrap_summary.json"),
    )
    parser.add_argument(
        "--materialized-matrix-dir",
        type=Path,
        default=Path("outputs/next_axis_after_v9_matrix_bootstrap/default/materialized_next_axis_after_v9_matrix"),
    )
    parser.add_argument(
        "--v10-inputs-dir",
        type=Path,
        default=Path("outputs/next_axis_after_v8_matrix_dry_run/default/materialized_next_axis_after_v8_matrix_inputs"),
    )
...
def main() -> int:
    args = parse_args()
    try:
        result = build_next_axis_after_v9_matrix_dry_run(
            ...
            v10_inputs_dir=args.v10_inputs_dir,
        ...
        print(f"TriScope-LLM next-axis-after-v9 matrix dry-run failed: {exc}")
        return 1
    print("TriScope-LLM next-axis-after-v9 matrix dry-run complete")
