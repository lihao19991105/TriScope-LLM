#!/usr/bin/env python3
"""CLI for route_c frozen semantic minimal real batched continuous regression."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory to write stage-154 artifacts.")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    from src.eval.route_c_frozen_semantic_minimal_real_batched_continuous_regression import (
        build_route_c_frozen_semantic_minimal_real_batched_continuous_regression,
    )

    try:
        result = build_route_c_frozen_semantic_minimal_real_batched_continuous_regression(
        ...
        print(f"TriScope-LLM route_c minimal real batched continuous regression failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("route_c frozen semantic minimal real batched continuous regression artifacts written to:")
