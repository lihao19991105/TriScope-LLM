#!/usr/bin/env python3
"""Validate pilot analysis artifacts and optionally compare repeatability."""
...
from src.eval.pilot_analysis_checks import compare_pilot_analysis_runs, validate_pilot_analysis


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM pilot analysis run, "
            "and optionally compare it against a second run for repeatability."
        ),
    )
    parser.add_argument("--run-dir", type=Path, required=True, help="Primary pilot analysis run directory.")
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
        default=None,
        help="Optional second pilot analysis run directory used for repeatability comparison.",
    )
...
def main() -> int:
    ...
    acceptance = validate_pilot_analysis(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_pilot_analysis_runs(
    ...
    log_lines = [
        "TriScope-LLM pilot analysis validation",
    ...
    print("TriScope-LLM pilot analysis validation complete")
