...
mkdir -p logs outputs/dualscope_qwen2p5_7b_external_gpu_generation/default
log_path="logs/dualscope_external_gpu_generation_$(date +%Y%m%d_%H%M%S).log"
...
pid=$!
echo "Started DualScope external GPU generation. PID: ${pid}"
echo "Log: ${log_path}"
echo "Progress: ps -ef | grep build_dualscope_qwen2p5_7b_external_gpu_generation | grep -v grep"
