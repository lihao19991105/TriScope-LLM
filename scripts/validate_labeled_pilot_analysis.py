#!/usr/bin/env python3
"""Validate labeled-pilot analysis artifacts and optionally compare repeatability."""
...
from src.eval.labeled_pilot_analysis_checks import compare_labeled_pilot_analysis, validate_labeled_pilot_analysis


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate labeled-pilot analysis artifacts and optionally compare repeatability.")
...
def main() -> int:
    ...
    acceptance = validate_labeled_pilot_analysis(args.run_dir)
    acceptance_path = args.output_dir / "artifact_acceptance.json"
    write_json(acceptance_path, acceptance)
    repeatability = None
    if args.compare_run_dir is not None:
        repeatability = compare_labeled_pilot_analysis(args.run_dir, args.compare_run_dir)
        write_json(args.output_dir / "repeatability_summary.json", repeatability)
    log_lines = [
        "TriScope-LLM labeled-pilot analysis validation",
    ...
    log_path.write_text("\n".join(log_lines) + "\n", encoding="utf-8")
    print("TriScope-LLM labeled-pilot analysis validation complete")
