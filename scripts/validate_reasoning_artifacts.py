#!/usr/bin/env python3
"""Validate reasoning artifacts and optionally compare repeatability."""
...
from src.features.reasoning_artifact_checks import (
    compare_reasoning_runs,
    validate_reasoning_run_artifacts,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM reasoning run and "
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Primary reasoning run directory to validate.",
    )
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
...
def main() -> int:
    ...
    acceptance = validate_reasoning_run_artifacts(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_reasoning_runs(args.run_dir, args.compare_run_dir)
        write_json(args.output_dir / "repeatability_summary.json", repeatability_payload)

    log_lines = [
        "TriScope-LLM reasoning artifact validation",
    ...
    print("TriScope-LLM reasoning artifact validation complete")
