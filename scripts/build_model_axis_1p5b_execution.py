#!/usr/bin/env python3
"""CLI for the 1.5B model-axis minimal execution."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the 1.5B model-axis minimal execution.")
    ...
    parser.add_argument("--execution-gate-path", type=Path, default=Path("outputs/model_axis_1p5b_dry_run/default/model_axis_1p5b_execution_gate.json"))
    parser.add_argument("--bootstrap-materialized-inputs-dir", type=Path, default=Path("outputs/model_axis_1p5b_bootstrap/default/materialized_model_axis_1p5b_inputs"))
    parser.add_argument("--dry-run-materialized-inputs-dir", type=Path, default=Path("outputs/model_axis_1p5b_dry_run/default/materialized_model_axis_1p5b_dry_run_inputs"))
    parser.add_argument("--reference-route-b-dataset-path", type=Path, default=Path("outputs/rerun_route_b_on_labeled_split_v6/default/route_b_v6_dataset.jsonl"))
    parser.add_argument("--reference-route-b-slice-path", type=Path, default=Path("outputs/rerun_route_b_on_labeled_split_v6/default/materialized_route_b_v6_inputs/csqa_reasoning_pilot_slice.jsonl"))
...
def main() -> int:
    args = parse_args()
    from src.eval.model_axis_1p5b_execution import build_model_axis_1p5b_execution

    try:
        result = build_model_axis_1p5b_execution(
        ...
        print(f"TriScope-LLM model-axis 1.5B execution failed: {exc}")
        return 1
    print("TriScope-LLM model-axis 1.5B execution complete")
