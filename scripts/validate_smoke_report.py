#!/usr/bin/env python3
"""Validate smoke report artifacts and optionally compare repeatability."""
...
from src.eval.smoke_report_checks import compare_smoke_reports, validate_smoke_report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM smoke report and "
            "optionally compare it against a second report for repeatability."
        ),
    )
    parser.add_argument("--run-dir", type=Path, required=True, help="Primary smoke report output directory.")
    parser.add_argument("--compare-run-dir", type=Path, default=None, help="Optional second smoke report directory used for repeatability comparison.")
...
def main() -> int:
    ...
    acceptance = validate_smoke_report(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_smoke_reports(args.run_dir, args.compare_run_dir)
        write_json(args.output_dir / "repeatability_summary.json", repeatability_payload)

    log_lines = [
        "TriScope-LLM smoke report validation",
    ...
    print("TriScope-LLM smoke report validation complete")
