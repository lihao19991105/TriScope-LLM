#!/usr/bin/env python3
"""CLI for the next-axis-after-v6 matrix v7 dry-run."""
...
from src.eval.next_axis_after_v6_matrix_dry_run import build_next_axis_after_v6_matrix_dry_run


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build the refined-fusion-support-ablation-aware next-axis-after-v6 matrix dry-run."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/next_axis_after_v6_matrix_bootstrap/default/next_axis_after_v6_bootstrap_summary.json"),
    )
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    try:
        result = build_next_axis_after_v6_matrix_dry_run(
            ...
            v7_inputs_dir=args.v7_inputs_dir,
        ...
        print(f"TriScope-LLM next-axis-after-v6 matrix dry-run failed: {exc}")
        return 1
    print("TriScope-LLM next-axis-after-v6 matrix dry-run complete")
