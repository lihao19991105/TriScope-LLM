#!/usr/bin/env python3
"""Build route B rerun on labeled split v5."""
...
from src.eval.rerun_route_b_on_labeled_split_v5 import materialize_route_b_v5, run_route_b_v5


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route B on labeled split v5.")
    parser.add_argument(
        "--v5-inputs-dir",
        type=Path,
        default=Path("outputs/post_v4_next_step_bootstrap/default/materialized_post_v4_inputs"),
    )
    parser.add_argument(
        "--route-c-v5-run-summary-path",
        type=Path,
        default=Path("outputs/rerun_route_c_on_labeled_split_v5/default/route_c_v5_run_summary.json"),
    )
...
def main() -> int:
    ...
    try:
        materialized = materialize_route_b_v5(
            v5_inputs_dir=args.v5_inputs_dir,
            route_c_v5_run_summary_path=args.route_c_v5_run_summary_path,
            output_dir=args.output_dir,
        )
        executed = run_route_b_v5(
            ...
            v5_inputs_dir=Path(materialized["output_paths"]["materialized_inputs_dir"]),
            output_dir=args.output_dir,
            seed=args.seed,
        ...
        print(f"TriScope-LLM route B on labeled split v5 failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route B on labeled split v5 complete")
