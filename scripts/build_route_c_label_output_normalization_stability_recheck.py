#!/usr/bin/env python3
"""Build repeated same-setting stability reruns for route_c output normalization recovery."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build route_c output normalization stability recheck artifacts."
    )
    parser.add_argument(
    ...
    parser.add_argument("--rerun-count", type=int, default=3)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--label-threshold", type=float, default=0.5)
    parser.add_argument(
        "--label-parse-mode",
        type=str,
        default="robust_prefix",
...
def main() -> int:
    args = parse_args()
    from src.eval.route_c_label_output_normalization_stability_recheck import (
        build_route_c_label_output_normalization_stability_recheck,
    )

    try:
        result = build_route_c_label_output_normalization_stability_recheck(
        ...
        print(f"TriScope-LLM route_c output normalization stability recheck failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route_c output normalization stability recheck complete")
