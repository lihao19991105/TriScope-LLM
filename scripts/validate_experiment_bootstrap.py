#!/usr/bin/env python3
"""Validate real-experiment bootstrap artifacts and optionally compare repeatability."""
...
from src.eval.experiment_bootstrap_checks import (
    compare_experiment_bootstrap_runs,
    validate_experiment_bootstrap,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM real-experiment bootstrap run, "
            "and optionally compare it against a second run for repeatability."
        ),
    )
    parser.add_argument("--run-dir", type=Path, required=True, help="Primary experiment bootstrap run directory.")
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
        default=None,
        help="Optional second experiment bootstrap run directory used for repeatability comparison.",
    )
...
def main() -> int:
    ...
    acceptance = validate_experiment_bootstrap(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_experiment_bootstrap_runs(
    ...
    log_lines = [
        "TriScope-LLM real experiment bootstrap validation",
    ...
    print("TriScope-LLM real experiment bootstrap validation complete")
