#!/usr/bin/env python3
"""Build route_c refined stability confirmation artifacts for model-axis 1.5B."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build model-axis 1.5B route_c refined stability artifacts.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--route-c-refinement-dir",
        type=Path,
        default=Path("outputs/model_axis_1p5b_route_c_refinement/default"),
    )
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    from src.eval.model_axis_1p5b_route_c_refined_stability import build_model_axis_1p5b_route_c_refined_stability

    try:
        result = build_model_axis_1p5b_route_c_refined_stability(
            route_c_refined_execution_dir=args.route_c_refined_execution_dir,
            route_c_refinement_dir=args.route_c_refinement_dir,
        ...
        print(f"TriScope-LLM model-axis 1.5B route_c refined stability failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM model-axis 1.5B route_c refined stability complete")
