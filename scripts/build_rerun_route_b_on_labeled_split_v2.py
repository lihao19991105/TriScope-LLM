#!/usr/bin/env python3
"""Build route B rerun on labeled split v2."""
...
from src.eval.rerun_route_b_on_labeled_split_v2 import (
    materialize_route_b_v2,
    run_route_b_v2,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route B on labeled split v2.")
    parser.add_argument(
        "--v2-inputs-dir",
        type=Path,
        default=Path("outputs/post_symmetric_next_step_bootstrap/default/materialized_post_symmetric_inputs"),
    )
    parser.add_argument(
        "--route-c-v2-run-summary",
        type=Path,
        default=Path("outputs/rerun_route_c_on_labeled_split_v2/default/route_c_v2_run_summary.json"),
    )
...
def main() -> int:
    ...
    try:
        materialized = materialize_route_b_v2(
            v2_inputs_dir=args.v2_inputs_dir,
            route_c_v2_run_summary_path=args.route_c_v2_run_summary,
            output_dir=args.output_dir,
        )
        executed = run_route_b_v2(
            ...
            v2_inputs_dir=Path(materialized["output_paths"]["materialized_inputs_dir"]),
            output_dir=args.output_dir,
            seed=args.seed,
        ...
        print(f"TriScope-LLM route B on labeled split v2 failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route B on labeled split v2 complete")
