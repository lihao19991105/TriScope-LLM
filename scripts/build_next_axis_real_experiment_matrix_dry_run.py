#!/usr/bin/env python3
"""CLI for the next-axis real-experiment matrix v4 dry-run."""
...
from src.eval.next_axis_real_experiment_matrix_dry_run import (
    build_next_axis_real_experiment_matrix_dry_run,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build the refined-fusion-aware next-axis real-experiment matrix dry-run."
    )
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    try:
        result = build_next_axis_real_experiment_matrix_dry_run(
        ...
        print(f"TriScope-LLM next-axis real-experiment matrix dry-run failed: {exc}")
        return 1
    print("TriScope-LLM next-axis real-experiment matrix dry-run complete")
