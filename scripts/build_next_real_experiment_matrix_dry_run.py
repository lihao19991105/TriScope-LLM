#!/usr/bin/env python3
"""Build the richer next real-experiment matrix dry-run."""
...
from src.eval.next_real_experiment_matrix_dry_run import (
    build_next_real_experiment_matrix_dry_run,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the richer next real-experiment matrix dry-run.")
    parser.add_argument(
...
def main() -> int:
    args = parse_args()
    try:
        result = build_next_real_experiment_matrix_dry_run(
        ...
        print(f"TriScope-LLM next real-experiment matrix dry-run failed: {exc}")
        return 1
    print("TriScope-LLM next real-experiment matrix dry-run complete")
