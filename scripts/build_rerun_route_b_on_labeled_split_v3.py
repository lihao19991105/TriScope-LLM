#!/usr/bin/env python3
"""Build route B rerun on labeled split v3."""
...
from src.eval.rerun_route_b_on_labeled_split_v3 import materialize_route_b_v3, run_route_b_v3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route B on labeled split v3.")
    parser.add_argument(
        "--v3-inputs-dir",
        type=Path,
        default=Path("outputs/post_v2_next_step_bootstrap/default/materialized_post_v2_inputs"),
    )
    parser.add_argument(
        "--route-c-v3-run-summary-path",
        type=Path,
        default=Path("outputs/rerun_route_c_on_labeled_split_v3/default/route_c_v3_run_summary.json"),
    )
...
def main() -> int:
    ...
    try:
        materialized = materialize_route_b_v3(
            v3_inputs_dir=args.v3_inputs_dir,
            route_c_v3_run_summary_path=args.route_c_v3_run_summary_path,
            output_dir=args.output_dir,
        )
        executed = run_route_b_v3(
            ...
            v3_inputs_dir=Path(materialized["output_paths"]["materialized_inputs_dir"]),
            output_dir=args.output_dir,
            seed=args.seed,
        ...
        print(f"TriScope-LLM route B on labeled split v3 failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route B on labeled split v3 complete")
