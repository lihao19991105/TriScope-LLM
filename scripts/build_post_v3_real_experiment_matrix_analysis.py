#!/usr/bin/env python3
"""Build the post-v3 real-experiment matrix analysis."""
...
from src.eval.post_v3_real_experiment_matrix_analysis import (
    build_post_v3_real_experiment_matrix_analysis,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the post-v3 real-experiment matrix analysis.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--post-next-matrix-execution-metrics-path",
        type=Path,
        default=Path("outputs/post_next_real_experiment_matrix_execution/default/post_next_matrix_execution_metrics.json"),
    )
    parser.add_argument(
        "--post-next-matrix-fusion-summary-path",
        type=Path,
        default=Path("outputs/post_next_real_experiment_matrix_execution/default/post_next_matrix_fusion_summary.json"),
    )
    parser.add_argument(
        "--post-next-matrix-fusion-cell-candidate-summary-path",
        type=Path,
        default=Path("outputs/post_next_real_experiment_matrix_execution/default/post_next_matrix_fusion_cell_candidate_summary.json"),
    )
...
def main() -> int:
    args = parse_args()
    try:
        result = build_post_v3_real_experiment_matrix_analysis(
            ...
            post_next_matrix_execution_metrics_path=args.post_next_matrix_execution_metrics_path,
            post_next_matrix_fusion_summary_path=args.post_next_matrix_fusion_summary_path,
            post_next_matrix_fusion_cell_candidate_summary_path=args.post_next_matrix_fusion_cell_candidate_summary_path,
        ...
        print(f"TriScope-LLM post-v3 real-experiment matrix analysis failed: {exc}")
        return 1
    print("TriScope-LLM post-v3 real-experiment matrix analysis complete")
