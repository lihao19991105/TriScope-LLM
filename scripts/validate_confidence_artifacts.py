#!/usr/bin/env python3
"""Validate confidence artifacts and optionally compare repeatability."""
...
from src.features.confidence_artifact_checks import (
    compare_confidence_runs,
    validate_confidence_run_artifacts,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM confidence run and "
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Primary confidence run directory to validate.",
    )
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
...
def main() -> int:
    ...
    acceptance = validate_confidence_run_artifacts(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_confidence_runs(args.run_dir, args.compare_run_dir)
        write_json(args.output_dir / "repeatability_summary.json", repeatability_payload)

    log_lines = [
        "TriScope-LLM confidence artifact validation",
    ...
    print("TriScope-LLM confidence artifact validation complete")
