#!/usr/bin/env python3
"""CLI for route_c frozen semantic minimal continuous-execution validation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory to write stage-152 artifacts.")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    from src.eval.route_c_frozen_semantic_minimal_continuous_execution_validation import (
        build_route_c_frozen_semantic_minimal_continuous_execution_validation,
    )

    try:
        result = build_route_c_frozen_semantic_minimal_continuous_execution_validation(
        ...
        print(f"TriScope-LLM route_c minimal continuous execution validation failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("route_c frozen semantic minimal continuous execution validation artifacts written to:")
