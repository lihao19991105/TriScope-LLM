#!/usr/bin/env python3
"""Build route B rerun on labeled split v4."""
...
from src.eval.rerun_route_b_on_labeled_split_v4 import materialize_route_b_v4, run_route_b_v4


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route B on labeled split v4.")
    parser.add_argument(
        "--v4-inputs-dir",
        type=Path,
    ...
    parser.add_argument(
        "--route-c-v4-run-summary-path",
        type=Path,
        default=Path("outputs/rerun_route_c_on_labeled_split_v4/default/route_c_v4_run_summary.json"),
    )
...
def main() -> int:
    ...
    try:
        materialized = materialize_route_b_v4(
            v4_inputs_dir=args.v4_inputs_dir,
            route_c_v4_run_summary_path=args.route_c_v4_run_summary_path,
            output_dir=args.output_dir,
        )
        executed = run_route_b_v4(
            ...
            v4_inputs_dir=Path(materialized["output_paths"]["materialized_inputs_dir"]),
            output_dir=args.output_dir,
            seed=args.seed,
        ...
        print(f"TriScope-LLM route B on labeled split v4 failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM route B on labeled split v4 complete")
