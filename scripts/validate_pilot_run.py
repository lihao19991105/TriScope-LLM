#!/usr/bin/env python3
"""Validate pilot run artifacts and optionally compare repeatability."""
...
from src.eval.pilot_run_checks import compare_pilot_runs, validate_pilot_run


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the required artifact set for a TriScope-LLM pilot run, "
            "and optionally compare it against a second pilot run for repeatability."
        ),
    )
    parser.add_argument("--run-dir", type=Path, required=True, help="Primary pilot run directory.")
    parser.add_argument(
        "--compare-run-dir",
        type=Path,
        default=None,
        help="Optional second pilot run directory used for repeatability comparison.",
    )
...
def main() -> int:
    ...
    acceptance = validate_pilot_run(args.run_dir)
    ...
    if args.compare_run_dir is not None:
        repeatability_payload = compare_pilot_runs(
    ...
    log_lines = [
        "TriScope-LLM pilot run validation",
    ...
    print("TriScope-LLM pilot run validation complete")
