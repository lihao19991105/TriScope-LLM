#!/usr/bin/env python3
"""Build larger-split route C rerun."""
...
from src.eval.rerun_route_c_on_larger_split import (
    materialize_larger_split_route_c,
    run_larger_split_route_c,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rerun route C on the larger labeled split.")
    parser.add_argument(
        "--larger-inputs-dir",
        type=Path,
    ...
    parser.add_argument(
        "--recommendation-path",
        type=Path,
        default=Path("outputs/post_rerun_comparison/default/post_rerun_next_step_recommendation.json"),
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"TriScope-LLM larger-split route C rerun failed: {exc}")
        print(f"Failure summary: {failure_path}")
        return 1

    print("TriScope-LLM larger-split route C rerun complete")
