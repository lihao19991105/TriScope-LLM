"""Validation helpers for more-natural-label bootstrap artifacts."""
...
def validate_more_natural_label_bootstrap(
    run_dir: Path,
    ...
    selection_path = run_dir / "more_natural_label_selection.json"
    definition_path = run_dir / "more_natural_label_definition.json"
    readiness_path = run_dir / "more_natural_label_readiness_summary.json"
    dataset_path = run_dir / "more_natural_labeled_dataset.jsonl"
    summary_path = run_dir / "more_natural_label_summary.json"
    supervised_readiness_path = run_dir / "more_natural_supervised_readiness_summary.json"
    predictions_path = run_dir / "more_natural_logistic_predictions.jsonl"
    logistic_summary_path = run_dir / "more_natural_logistic_summary.json"

    selection = load_json(selection_path)
    definition = load_json(definition_path)
    readiness = load_json(readiness_path)
    summary = load_json(summary_path)
    supervised_readiness = load_json(supervised_readiness_path)
    ...
    acceptance = {
        ...
        "artifact_checks": [
            ...
            {"artifact_name": "supervised_readiness", "path": str(supervised_readiness_path.resolve()), "exists": supervised_readiness_path.is_file()},
        ...
        "field_checks": {
            ...
            "definition_name_correct": definition.get("label_name") == "task_correctness_violation_label",
            ...
            "supervised_readiness_pass": supervised_readiness.get("summary_status") == "PASS",
    ...
    if compare_run_dir is not None:
        compare_summary = load_json(compare_run_dir / "more_natural_label_summary.json")
        compare_logistic = load_json(compare_run_dir / "more_natural_logistic_summary.json")
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM more-natural-label bootstrap validation",
