"""Rerun route B on the labeled split v3 substrate."""
...
SCHEMA_VERSION = "triscopellm/rerun-route-b-on-labeled-split-v3/v1"
...
def materialize_route_b_v3(
    v3_inputs_dir: Path,
    route_c_v3_run_summary_path: Path,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)

    route_c_v3_run_summary = load_json(route_c_v3_run_summary_path)
    if route_c_v3_run_summary.get("summary_status") != "PASS":
        raise ValueError("043 expects route C v3 run summary to be PASS before route B v3 rerun.")
    ...
    for path in required_inputs:
        if not path.is_file():
            raise ValueError(f"Route B v3 input not found: `{path}`.")

    materialized_inputs_dir = output_dir / "materialized_route_b_v3_inputs"
    ...
    plan = {
        ...
        "selected_from": str(route_c_v3_run_summary_path.resolve()),
        "success_criterion": [
            "materialize route B on labeled split v3",
            "produce a non-empty sample-level more-natural dataset on v3",
            "run route B v3 logistic bootstrap",
        ],
        "known_risks": [
    ...
    label_definition = {
        ...
        "row_expansion_rule": "each v3 base sample emits one sample-level row after multi-modal alignment",
        "label_limitations": [
    ...
    readiness_summary = {
        ...
        "expected_row_capacity": {
            "route_b_v3_alignment_rows": reasoning_rows,
            "route_b_v3_dataset_rows": reasoning_rows,
        },
        "notes": [
            "This rerun keeps route B's more-natural supervision semantics while moving it onto labeled split v3.",
            "It extends route B from the 30-row v2 split to the 40-row v3 split.",
        ],
    }

    write_json(output_dir / "route_b_v3_plan.json", plan)
    write_json(output_dir / "route_b_v3_label_definition.json", label_definition)
    write_json(output_dir / "route_b_v3_readiness_summary.json", readiness_summary)
    write_json(
        output_dir / "config_snapshot.json",
        {
            "schema_version": SCHEMA_VERSION,
            "v3_inputs_dir": str(v3_inputs_dir.resolve()),
            "route_c_v3_run_summary_path": str(route_c_v3_run_summary_path.resolve()),
    ...
    (output_dir / "build.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route B on labeled split v3 materialization",
                "chosen_route=B",
    ...
    return {
        ...
        "output_paths": {
            "plan": str((output_dir / "route_b_v3_plan.json").resolve()),
            "label_definition": str((output_dir / "route_b_v3_label_definition.json").resolve()),
            "readiness_summary": str((output_dir / "route_b_v3_readiness_summary.json").resolve()),
            "materialized_inputs_dir": str(materialized_inputs_dir.resolve()),
...
def run_route_b_v3(
    ...
    v3_slice_path = v3_inputs_dir / "csqa_reasoning_pilot_slice.jsonl"

    v3_budget = count_jsonl_rows(reasoning_query_file)
    if v3_budget <= 0:
        raise ValueError("Route B v3 requires non-empty reasoning query contracts.")

    reasoning_dir = output_dir / "route_b_v3_reasoning"
    confidence_dir = output_dir / "route_b_v3_confidence"
    illumination_dir = output_dir / "route_b_v3_illumination"
    ...
    reasoning_features = extract_reasoning_features(
        ...
        run_id="route_b_v3_reasoning",
    )
    write_json(
        reasoning_dir / "route_b_v3_reasoning_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    confidence_features = extract_confidence_features(
        ...
        run_id="route_b_v3_confidence",
        high_confidence_threshold=0.10,
    )
    write_json(
        confidence_dir / "route_b_v3_confidence_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    illumination_features = extract_illumination_features(
        ...
        run_id="route_b_v3_illumination",
    )
    write_json(
        illumination_dir / "route_b_v3_illumination_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    fusion_result = build_fusion_dataset(
        ...
        output_dir=output_dir / "route_b_v3_real_pilot_fusion",
        join_mode="inner",
    )
    ...
    dataset_result = build_more_natural_labeled_dataset(
        ...
        pilot_slice_path=v3_slice_path,
        output_dir=output_dir / "route_b_v3_dataset_dir",
        fusion_profile="more_natural_label_real_pilot_v3",
        run_id="v3",
    )
    logistic_result = run_more_natural_logistic(
        dataset_path=output_dir / "route_b_v3_dataset_dir" / "more_natural_labeled_dataset.jsonl",
        output_dir=output_dir / "route_b_v3_run_dir",
        fusion_profile="more_natural_label_real_pilot_v3",
        run_id="v3",
    ...
    copy_artifact(
        output_dir / "route_b_v3_dataset_dir" / "more_natural_labeled_dataset.jsonl",
        output_dir / "route_b_v3_dataset.jsonl",
    )
    copy_artifact(
        output_dir / "route_b_v3_dataset_dir" / "more_natural_labeled_dataset.csv",
        output_dir / "route_b_v3_dataset.csv",
    )
    copy_artifact(
        output_dir / "route_b_v3_dataset_dir" / "more_natural_label_summary.json",
        output_dir / "route_b_v3_summary.json",
    )
    copy_artifact(
        output_dir / "route_b_v3_run_dir" / "more_natural_logistic_predictions.jsonl",
        output_dir / "route_b_v3_logistic_predictions.jsonl",
    )
    copy_artifact(
        output_dir / "route_b_v3_run_dir" / "more_natural_logistic_summary.json",
        output_dir / "route_b_v3_logistic_summary.json",
    )
    copy_artifact(
        output_dir / "route_b_v3_run_dir" / "more_natural_model_metadata.json",
        output_dir / "route_b_v3_model_metadata.json",
    )
    ...
    run_summary = {
        ...
        "schema_version": SCHEMA_VERSION,
        "chosen_route": "B",
        "route_name": "more_natural_supervision_proxy_v3",
        "route_b_v3_alignment": alignment_summary,
        "route_b_v3_summary": dataset_result["summary"],
        "route_b_v3_logistic_summary": logistic_result["summary"],
        "notes": [
            "This is route B rerun on the 40-row labeled split v3.",
    ...
    write_json(output_dir / "route_b_v3_run_summary.json", run_summary)
    (output_dir / "run.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route B on labeled split v3 rerun",
                "chosen_route=B",
                f"v3_budget={v3_budget}",
                f"alignment_rows={alignment_summary['num_rows']}",
                f"route_b_v3_rows={dataset_result['summary']['num_rows']}",
                f"route_b_v3_predictions={logistic_result['summary']['num_predictions']}",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    return {
        ...
        "output_paths": {
            "dataset_jsonl": str((output_dir / "route_b_v3_dataset.jsonl").resolve()),
            "dataset_csv": str((output_dir / "route_b_v3_dataset.csv").resolve()),
            "summary": str((output_dir / "route_b_v3_summary.json").resolve()),
            "predictions": str((output_dir / "route_b_v3_logistic_predictions.jsonl").resolve()),
            "logistic_summary": str((output_dir / "route_b_v3_logistic_summary.json").resolve()),
            "model_metadata": str((output_dir / "route_b_v3_model_metadata.json").resolve()),
            "run_summary": str((output_dir / "route_b_v3_run_summary.json").resolve()),
