"""Rerun route C on the labeled split v4 substrate."""
...
SCHEMA_VERSION = "triscopellm/rerun-route-c-on-labeled-split-v4/v1"
...
def materialize_route_c_v4(
    ...
    if bootstrap_plan.get("chosen_next_step") != "prepare_larger_labeled_split_v4":
        raise ValueError("046 expects `chosen_next_step` to be `prepare_larger_labeled_split_v4`.")
    ...
    required_inputs = [
        ...
        v4_inputs_dir / "labeled_illumination_query_contracts.jsonl",
        v4_inputs_dir / "csqa_reasoning_pilot_slice.jsonl",
    ]
    for path in required_inputs:
        if not path.is_file():
            raise ValueError(f"Route C v4 input not found: `{path}`.")

    materialized_inputs_dir = output_dir / "materialized_route_c_v4_inputs"
    ...
    plan = {
        ...
        "success_criterion": [
            "materialize route C on labeled split v4",
            "produce a non-empty contract-level benchmark-truth-leaning dataset on v4",
            "run route C v4 logistic bootstrap",
        ],
        "known_risks": [
    ...
    label_definition = {
        ...
        "row_expansion_rule": "each v4 base sample emits control/targeted contract rows through labeled illumination contracts",
        "label_limitations": [
    ...
    readiness_summary = {
        ...
        "expected_row_capacity": {
            "route_c_v4_alignment_rows": reasoning_rows,
            "route_c_v4_contract_rows": labeled_rows,
        },
        "notes": [
            "This rerun keeps route C's benchmark-truth-leaning supervision semantics while moving it onto labeled split v4.",
            "It extends route C from the 40-row v3 split to the 50-row labeled split v4.",
        ],
    }

    write_json(output_dir / "route_c_v4_plan.json", plan)
    write_json(output_dir / "route_c_v4_label_definition.json", label_definition)
    write_json(output_dir / "route_c_v4_readiness_summary.json", readiness_summary)
    write_json(
    ...
    (output_dir / "build.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route C on labeled split v4 materialization",
                "chosen_route=C",
    ...
    return {
        ...
        "output_paths": {
            "plan": str((output_dir / "route_c_v4_plan.json").resolve()),
            "label_definition": str((output_dir / "route_c_v4_label_definition.json").resolve()),
            "readiness_summary": str((output_dir / "route_c_v4_readiness_summary.json").resolve()),
            "materialized_inputs_dir": str(materialized_inputs_dir.resolve()),
...
def run_route_c_v4(
    ...
    labeled_illumination_query_file = v4_inputs_dir / "labeled_illumination_query_contracts.jsonl"

    v4_budget = count_jsonl_rows(reasoning_query_file)
    if v4_budget <= 0:
        raise ValueError("Route C v4 requires non-empty reasoning query contracts.")

    reasoning_dir = output_dir / "route_c_v4_reasoning"
    confidence_dir = output_dir / "route_c_v4_confidence"
    illumination_dir = output_dir / "route_c_v4_illumination"
    labeled_illumination_dir = output_dir / "route_c_v4_labeled_illumination"
    ...
    reasoning_features = extract_reasoning_features(
        ...
        run_id="route_c_v4_reasoning",
    )
    write_json(
        reasoning_dir / "route_c_v4_reasoning_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    confidence_features = extract_confidence_features(
        ...
        run_id="route_c_v4_confidence",
        high_confidence_threshold=0.10,
    )
    write_json(
        confidence_dir / "route_c_v4_confidence_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    illumination_features = extract_illumination_features(
        ...
        run_id="route_c_v4_illumination",
    )
    write_json(
        illumination_dir / "route_c_v4_illumination_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    labeled_features = extract_illumination_features(
        ...
        run_id="route_c_v4_labeled_illumination",
    )
    write_json(
        labeled_illumination_dir / "route_c_v4_labeled_illumination_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    fusion_result = build_fusion_dataset(
        ...
        output_dir=output_dir / "route_c_v4_real_pilot_fusion",
        join_mode="inner",
    )
    ...
    dataset_result = build_benchmark_truth_leaning_dataset(
        ...
        output_dir=output_dir / "route_c_v4_dataset_dir",
        fusion_profile="benchmark_truth_leaning_real_pilot_v4",
        run_id="v4",
    )
    logistic_result = run_benchmark_truth_leaning_logistic(
        dataset_path=output_dir / "route_c_v4_dataset_dir" / "benchmark_truth_leaning_dataset.jsonl",
        output_dir=output_dir / "route_c_v4_run_dir",
        fusion_profile="benchmark_truth_leaning_real_pilot_v4",
        run_id="v4",
    ...
    copy_artifact(
        output_dir / "route_c_v4_dataset_dir" / "benchmark_truth_leaning_dataset.jsonl",
        output_dir / "route_c_v4_dataset.jsonl",
    )
    copy_artifact(
        output_dir / "route_c_v4_dataset_dir" / "benchmark_truth_leaning_dataset.csv",
        output_dir / "route_c_v4_dataset.csv",
    )
    copy_artifact(
        output_dir / "route_c_v4_dataset_dir" / "benchmark_truth_leaning_summary.json",
        output_dir / "route_c_v4_summary.json",
    )
    copy_artifact(
        output_dir / "route_c_v4_run_dir" / "benchmark_truth_leaning_logistic_predictions.jsonl",
        output_dir / "route_c_v4_logistic_predictions.jsonl",
    )
    copy_artifact(
        output_dir / "route_c_v4_run_dir" / "benchmark_truth_leaning_logistic_summary.json",
        output_dir / "route_c_v4_logistic_summary.json",
    )
    copy_artifact(
        output_dir / "route_c_v4_run_dir" / "benchmark_truth_leaning_model_metadata.json",
        output_dir / "route_c_v4_model_metadata.json",
    )
    ...
    run_summary = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "route_name": "benchmark_truth_leaning_supervision_proxy_v4",
        "route_c_v4_alignment": alignment_summary,
        "route_c_v4_summary": dataset_result["summary"],
        "route_c_v4_logistic_summary": logistic_result["summary"],
        "notes": [
            "This is route C rerun on the 50-row labeled split v4.",
    ...
    write_json(output_dir / "route_c_v4_run_summary.json", run_summary)
    (output_dir / "run.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route C on labeled split v4 rerun",
                f"v4_budget={v4_budget}",
                f"labeled_budget={labeled_budget}",
                f"alignment_rows={alignment_summary['num_rows']}",
                f"route_c_v4_rows={dataset_result['summary']['num_rows']}",
                f"route_c_v4_predictions={logistic_result['summary']['num_predictions']}",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    return {
        ...
        "output_paths": {
            "dataset_jsonl": str((output_dir / "route_c_v4_dataset.jsonl").resolve()),
            "dataset_csv": str((output_dir / "route_c_v4_dataset.csv").resolve()),
            "summary": str((output_dir / "route_c_v4_summary.json").resolve()),
            "predictions": str((output_dir / "route_c_v4_logistic_predictions.jsonl").resolve()),
            "logistic_summary": str((output_dir / "route_c_v4_logistic_summary.json").resolve()),
            "model_metadata": str((output_dir / "route_c_v4_model_metadata.json").resolve()),
            "run_summary": str((output_dir / "route_c_v4_run_summary.json").resolve()),
