"""Validation helpers for route B on labeled split v2 artifacts."""
...
def validate_rerun_route_b_on_labeled_split_v2(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    plan_path = run_dir / "route_b_v2_plan.json"
    definition_path = run_dir / "route_b_v2_label_definition.json"
    readiness_path = run_dir / "route_b_v2_readiness_summary.json"
    dataset_path = run_dir / "route_b_v2_dataset.jsonl"
    summary_path = run_dir / "route_b_v2_summary.json"
    predictions_path = run_dir / "route_b_v2_logistic_predictions.jsonl"
    logistic_summary_path = run_dir / "route_b_v2_logistic_summary.json"
    run_summary_path = run_dir / "route_b_v2_run_summary.json"
    ...
    acceptance = {
        ...
        "field_checks": {
            ...
            "definition_label_correct": definition.get("label_name") == "task_correctness_violation_label",
    ...
    if compare_run_dir is not None:
        compare_summary = load_json(compare_run_dir / "route_b_v2_summary.json")
        compare_logistic = load_json(compare_run_dir / "route_b_v2_logistic_summary.json")
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route B on labeled split v2 validation",
