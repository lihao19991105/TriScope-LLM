"""Bootstrap the next step after v4 symmetric rerun comparison."""
...
SCHEMA_VERSION = "triscopellm/post-v4-next-step-bootstrap/v1"

ADDITIONAL_ROWS_V5: list[dict[str, Any]] = [
    {
        "sample_id": "csqa-pilot-051",
        "question": "What would you use to unlock a front door?",
        "choices": [
            {"label": "A", "text": "house key"},
            {"label": "B", "text": "coffee mug"},
            {"label": "C", "text": "pencil case"},
            {"label": "D", "text": "blanket"},
        ],
        "answerKey": "A",
    },
    {
        "sample_id": "csqa-pilot-052",
        ...
        "choices": [
            ...
            {"label": "D", "text": "dish rack"},
        ],
        "answerKey": "A",
    },
    {
        "sample_id": "csqa-pilot-053",
        "question": "What would you use to cut wrapping paper neatly?",
        "choices": [
            {"label": "A", "text": "scissors"},
            {"label": "B", "text": "flashlight"},
            {"label": "C", "text": "shoe box"},
            {"label": "D", "text": "soap bar"},
        ],
        "answerKey": "A",
    },
    {
        "sample_id": "csqa-pilot-054",
        "question": "Where would you usually hang a wet towel to dry?",
        "choices": [
            {"label": "A", "text": "towel rack"},
            {"label": "B", "text": "oven drawer"},
    ...
    {
        "sample_id": "csqa-pilot-055",
        "question": "What would you use to stir soup in a pot?",
        "choices": [
            {"label": "A", "text": "spoon"},
            {"label": "B", "text": "shoelace"},
            {"label": "C", "text": "eraser"},
            {"label": "D", "text": "pillow"},
        ],
        "answerKey": "A",
    ...
    {
        "sample_id": "csqa-pilot-056",
        "question": "Where would you place dishes after washing them by hand?",
        "choices": [
            {"label": "A", "text": "drying rack"},
            {"label": "B", "text": "bedside table"},
            {"label": "C", "text": "closet floor"},
            {"label": "D", "text": "mailbox"},
        ],
        "answerKey": "A",
    },
    {
        "sample_id": "csqa-pilot-057",
        "question": "What would you use to sweep crumbs off the kitchen floor?",
        "choices": [
            {"label": "A", "text": "broom"},
            {"label": "B", "text": "pillowcase"},
            {"label": "C", "text": "remote control"},
            {"label": "D", "text": "keychain"},
        ],
        "answerKey": "A",
    },
    {
        "sample_id": "csqa-pilot-058",
        "question": "Where would you keep shampoo in a bathroom?",
        "choices": [
            {"label": "A", "text": "shower shelf"},
            {"label": "B", "text": "desk lamp"},
            {"label": "C", "text": "sock drawer"},
            {"label": "D", "text": "window curtain"},
        ],
        "answerKey": "A",
    },
    {
        "sample_id": "csqa-pilot-059",
        "question": "What would you use to carry groceries home from the store?",
        "choices": [
            {"label": "A", "text": "shopping bag"},
            {"label": "B", "text": "pillow"},
            {"label": "C", "text": "toothbrush"},
            {"label": "D", "text": "coat hanger"},
        ],
        "answerKey": "A",
    },
    {
        "sample_id": "csqa-pilot-060",
        "question": "Where would you usually place leftovers to keep them cool overnight?",
        "choices": [
            {"label": "A", "text": "refrigerator"},
            {"label": "B", "text": "garage shelf"},
            {"label": "C", "text": "desk drawer"},
...
def build_larger_split_v5_rows(current_v4_split_path: Path) -> list[dict[str, Any]]:
    base_rows = load_jsonl(current_v4_split_path)
    if len(base_rows) != 50:
        raise ValueError(f"Expected a 50-row v4 split at `{current_v4_split_path}`, found {len(base_rows)} rows.")

    all_rows: list[dict[str, Any]] = []
    for row in base_rows:
        metadata = dict(row.get("metadata", {}))
        metadata["slice_size"] = 60
        metadata["expansion_stage"] = "larger_labeled_split_v5_bootstrap"
        metadata["source_name"] = "commonsenseqa_style_curated_pilot_larger_v5"
        all_rows.append(
            {
        ...
        all_rows.append(
            {
                ...
                "metadata": {
                    "source_name": "commonsenseqa_style_curated_pilot_larger_v5",
                    "source_split": "pilot_larger_v5",
                    "slice_size": 60,
                    "expansion_stage": "larger_labeled_split_v5_bootstrap",
...
def run_bridge_dry_runs(
    ...
    summary = {
        ...
        "notes": [
            "These dry-run artifacts prove the v5 split contracts can still enter the stable probe CLIs without loading a model.",
            "They validate builder compatibility, not model behavior quality.",
        ],
    }
    write_json(output_dir / "post_v4_next_step_bridge_summary.json", summary)
    return summary


def build_post_v4_next_step_bootstrap(
    current_v4_inputs_dir: Path,
    ...
    if recommendation.get("recommended_next_step") != "prepare_larger_labeled_split_v5":
        raise ValueError("049 expects `recommended_next_step` to be `prepare_larger_labeled_split_v5`.")

    current_split_path = current_v4_inputs_dir / "csqa_reasoning_pilot_slice.jsonl"
    if not current_split_path.is_file():
        raise ValueError(f"Current v4 split input not found: `{current_split_path}`.")

    v5_rows = build_larger_split_v5_rows(current_split_path)
    materialized_inputs_dir = output_dir / "materialized_post_v4_inputs"
    materialized_inputs_dir.mkdir(parents=True, exist_ok=True)

    write_jsonl(output_dir / "larger_labeled_split_v5.jsonl", v5_rows)
    write_csv(output_dir / "larger_labeled_split_v5.csv", v5_rows)
    write_jsonl(materialized_inputs_dir / "csqa_reasoning_pilot_slice.jsonl", v5_rows)

    reasoning_contracts = build_reasoning_query_contracts(v5_rows)
    confidence_contracts = build_confidence_query_contracts_from_pilot_slice(v5_rows)
    illumination_contracts = build_illumination_query_contracts(v5_rows)
    labeled_contracts = build_labeled_query_contracts(v5_rows)
    ...
    plan = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "chosen_next_step": "prepare_larger_labeled_split_v5",
        "split_name": "larger_csqa_reasoning_pilot_local_v5",
        "row_count_target": 60,
        "base_sample_count_target": 60,
        "expected_follow_on_capacity": {
            "route_b_more_natural_rows": 60,
            "route_c_benchmark_truth_leaning_rows": 120,
            "route_a_controlled_rows": 120,
        },
    }
    readiness = {
        "summary_status": "PASS",
        ...
        "notes": [
            "This bootstrap raises the shared substrate from 50 rows to 60 rows.",
            "It remains local and curated, but is intended to unlock the next v5 rerun stage.",
        ],
    }
    summary = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "split_name": "larger_csqa_reasoning_pilot_local_v5",
        "num_rows": len(v5_rows),
        "num_new_rows": len(ADDITIONAL_ROWS_V5),
        "num_base_samples": len(v5_rows),
        "expected_follow_on_capacity": plan["expected_follow_on_capacity"],
        "notes": [
            "This v5 split remains a local curated split.",
            "Its purpose is to lift the shared substrate after v4 symmetric B/C comparison.",
        ],
    }

    write_json(output_dir / "post_v4_next_step_plan.json", plan)
    write_json(output_dir / "post_v4_next_step_readiness_summary.json", readiness)
    write_json(output_dir / "larger_labeled_split_v5_summary.json", summary)
    write_json(
        output_dir / "config_snapshot.json",
        {
            "schema_version": SCHEMA_VERSION,
            "current_v4_inputs_dir": str(current_v4_inputs_dir.resolve()),
    ...
    (output_dir / "build.log").write_text(
        "\n".join(
            [
                "TriScope-LLM post-v4 next-step bootstrap",
                "chosen_next_step=prepare_larger_labeled_split_v5",
                f"num_rows={len(v5_rows)}",
    ...
    return {
        ...
        "output_paths": {
            "plan": str((output_dir / "post_v4_next_step_plan.json").resolve()),
            "readiness": str((output_dir / "post_v4_next_step_readiness_summary.json").resolve()),
            "split_jsonl": str((output_dir / "larger_labeled_split_v5.jsonl").resolve()),
            "split_csv": str((output_dir / "larger_labeled_split_v5.csv").resolve()),
            "summary": str((output_dir / "larger_labeled_split_v5_summary.json").resolve()),
            "bridge_summary": str((output_dir / "post_v4_next_step_bridge_summary.json").resolve()),
