"""Bootstrap the next-axis-after-v8 real-experiment matrix."""
...
SCHEMA_VERSION = "triscopellm/next-axis-after-v8-matrix-bootstrap/v1"
...
def build_next_axis_after_v8_matrix_bootstrap(
    ...
    if recommendation.get("recommended_next_step") != "bootstrap_next_axis_after_v8_matrix":
        raise ValueError("093 expects recommendation to request next-axis-after-v8 bootstrap.")

    matrix_plan = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "chosen_next_step": "bootstrap_next_axis_after_v8_matrix",
    ...
    matrix_definition = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "matrix_name": "next_axis_after_v8_real_experiment_matrix_v9",
        ...
        "fusion_mode": "stress_refined_fusion_support_floor_invariance_contract",
        "output_expectations": [
            "route_b_summary",
            "route_c_summary",
            "fusion_summary",
        ...
        "expansion_notes": [
            "This next-axis-after-v8 matrix keeps dataset/model fixed and probes the support-ablation floor directly.",
            "The goal is to test whether the swept support-isolated floor is actually invariant once it is turned into an explicit floor probe.",
        ],
    }
    readiness = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "next_axis_after_v8_matrix_ready": True,
    ...
    write_json(output_dir / "next_axis_after_v8_matrix_plan.json", matrix_plan)
    write_json(output_dir / "next_axis_after_v8_matrix_definition.json", matrix_definition)
    write_json(output_dir / "next_axis_after_v8_matrix_readiness_summary.json", readiness)

    materialized_dir = output_dir / "materialized_next_axis_after_v8_matrix"
    ...
    write_json(output_dir / "next_axis_after_v8_input_contract.json", input_contract)
    write_json(output_dir / "next_axis_after_v8_bootstrap_summary.json", bootstrap_summary)
    return {
        "summary": bootstrap_summary,
        "output_paths": {
            "plan": str((output_dir / "next_axis_after_v8_matrix_plan.json").resolve()),
            "definition": str((output_dir / "next_axis_after_v8_matrix_definition.json").resolve()),
            "readiness": str((output_dir / "next_axis_after_v8_matrix_readiness_summary.json").resolve()),
            "contract": str((output_dir / "next_axis_after_v8_input_contract.json").resolve()),
            "summary": str((output_dir / "next_axis_after_v8_bootstrap_summary.json").resolve()),
        },
    }
