"""Package bounded Qwen2.5-7B semantic trigger smoke results."""
...
SCHEMA_VERSION = "dualscope/qwen2p5-7b-semantic-trigger-smoke-result-package/v1"
TASK_ID = "dualscope-qwen2p5-7b-semantic-trigger-smoke-result-package"
FINAL_VALIDATED = "Qwen2.5-7B semantic trigger smoke result package validated"
NEXT_TASK = "dualscope-qwen2p5-7b-behavior-shift-target-smoke-response-generation"
...
def write_report(path: Path, lines: list[str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(["# DualScope Semantic Trigger Smoke Result Package", "", *lines, ""]) + "\n", encoding="utf-8")


def build_semantic_trigger_smoke_result_package(
    *,
    response_dir: Path,
    metric_dir: Path,
    output_dir: Path,
    ...
    response_summary = read_json(response_dir / "semantic_trigger_smoke_summary.json")
    ...
    response_rows = read_jsonl_count(response_dir / "semantic_trigger_smoke_responses.jsonl")
    ...
    next_task = NEXT_TASK if final_verdict == FINAL_VALIDATED else "dualscope-qwen2p5-7b-semantic-trigger-smoke-result-package-repair"
    ...
    summary = {
        ...
        "limitations": [
            "Bounded semantic trigger smoke only; not full trigger-family performance.",
    ...
    verdict = {
        ...
        "final_verdict": final_verdict,
        "recommended_next_step": next_task,
        "single_verdict_policy": "one_of_semantic_trigger_smoke_result_package_validated__partially_validated__not_validated",
    }
    registry = {
        "task_id": TASK_ID,
    ...
    write_report(
        ...
        [
            ...
            f"- Semantic response rows: `{response_rows}`",
