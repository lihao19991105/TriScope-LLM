...
def _repair_next_step(summary: dict[str, Any]) -> tuple[str, str]:
    ...
    if "missing_labeled_pairs" in blocker_text or "missing_target_response_plan" in blocker_text:
...
def build_qwen2p5_7b_response_generation_repair(
    *,
    ...
    if dry_run:
        ...
        write_json(output_dir / "response_generation_repair_blockers.json", {"blockers": summary["blockers"]})
        (output_dir / "response_generation_repair_responses.jsonl").write_text("", encoding="utf-8")
        (output_dir / "response_generation_repair_report.md").write_text(
            "# Qwen2.5-7B Response Generation Repair\n\nDry-run only; no generation was attempted.\n",
            encoding="utf-8",
        )
        return summary
    ...
    if generation_summary.get("final_verdict") == "Qwen2.5-7B first-slice response generation validated" or generated_rows > 0:
        ...
        final_verdict = FINAL_PARTIAL
    next_task, blocker_type = _repair_next_step(generation_summary)
    ...
    summary = {
        ...
        "blockers": generation_summary.get("blockers") or [],
    ...
    rows_target = output_dir / "response_generation_repair_responses.jsonl"
    if rows_source.exists():
        rows_target.write_text(rows_source.read_text(encoding="utf-8"), encoding="utf-8")
    else:
        rows_target.write_text("", encoding="utf-8")
    write_json(
