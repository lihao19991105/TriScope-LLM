"""First minimal real-experiment matrix dry-run builder."""
...
SCHEMA_VERSION = "triscopellm/first-minimal-real-experiment-matrix-dry-run/v1"
...
def build_first_minimal_real_experiment_matrix_dry_run(
    ...
    if matrix_definition.get("summary_status") != "PASS":
        raise ValueError("Matrix definition must be PASS before matrix dry-run.")
    if matrix_bootstrap_summary.get("summary_status") != "PASS":
        raise ValueError("Matrix bootstrap summary must be PASS before matrix dry-run.")
    ...
    for path in [*required_matrix_inputs, *required_v6_inputs]:
        if not path.is_file():
            raise ValueError(f"Matrix dry-run input not found: `{path}`.")

    materialized_execution_dir = output_dir / "materialized_first_matrix_inputs"
    ...
    plan = {
        ...
        "dry_run_mode": "matrix_level_realistic_simulation",
        "seed": seed,
        "cell_count": 1,
        "required_modules": ["reasoning", "confidence", "illumination", "route_b", "route_c", "fusion_summary"],
    }
    contract = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        ...
        "cells": [
            {
                ...
                "routes": matrix_definition["routes"],
    ...
    write_json(output_dir / "first_matrix_dry_run_plan.json", plan)
    write_json(output_dir / "first_matrix_execution_contract.json", contract)
    write_json(output_dir / "first_matrix_readiness_summary.json", readiness)
    ...
    module_status = {
        ...
        "modules": {
            ...
            "fusion_summary": {"status": "PASS", "mode": "matrix_cell_alignment_summary"},
        },
    }
    cell_status = {
        "summary_status": "PASS",
    ...
    registry = {
        ...
        "matrix_name": matrix_bootstrap_summary["matrix_name"],
        "artifacts": {
            "plan": str((output_dir / "first_matrix_dry_run_plan.json").resolve()),
            "contract": str((output_dir / "first_matrix_execution_contract.json").resolve()),
            "module_status": str((output_dir / "first_matrix_module_status.json").resolve()),
            "cell_status": str((output_dir / "first_matrix_cell_status.json").resolve()),
        },
    }
    preview_rows = [
        {
    ...
    summary = {
        ...
        "cell_count": 1,
        "passed_cells": [cell_id],
        "blocked_cells": [],
        "passed_modules": list(module_status["modules"].keys()),
        "blocked_modules": [],
        "dataset_rows": len(dataset_rows),
    }
    write_json(output_dir / "first_matrix_module_status.json", module_status)
    write_json(output_dir / "first_matrix_cell_status.json", cell_status)
    write_json(output_dir / "first_matrix_dry_run_registry.json", registry)
    write_json(output_dir / "first_matrix_dry_run_summary.json", summary)
    write_jsonl(output_dir / "first_matrix_dry_run_preview.jsonl", preview_rows)
    (output_dir / "first_matrix_dry_run.log").write_text(
        "\n".join(
            [
                "TriScope-LLM first minimal real-experiment matrix dry-run",
    ...
    return {
        "summary": summary,
        "output_paths": {
            "summary": str((output_dir / "first_matrix_dry_run_summary.json").resolve()),
            "registry": str((output_dir / "first_matrix_dry_run_registry.json").resolve()),
            "module_status": str((output_dir / "first_matrix_module_status.json").resolve()),
            "cell_status": str((output_dir / "first_matrix_cell_status.json").resolve()),
        },
    }
