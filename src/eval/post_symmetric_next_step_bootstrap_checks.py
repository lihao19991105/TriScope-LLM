"""Validation helpers for post-symmetric next-step bootstrap artifacts."""
...
def validate_post_symmetric_next_step_bootstrap(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    plan_path = run_dir / "post_symmetric_next_step_plan.json"
    readiness_path = run_dir / "post_symmetric_next_step_readiness_summary.json"
    split_path = run_dir / "larger_labeled_split_v2.jsonl"
    summary_path = run_dir / "larger_labeled_split_v2_summary.json"
    bridge_path = run_dir / "post_symmetric_next_step_bridge_summary.json"
    ...
    acceptance = {
        ...
        "field_checks": {
            ...
            "chosen_next_step_is_v2": plan.get("chosen_next_step") == "prepare_larger_labeled_split_v2",
            "readiness_pass": readiness.get("summary_status") == "PASS",
            "split_has_30_rows": count_jsonl_rows(split_path) == 30,
    ...
    if compare_run_dir is not None:
        compare_summary = load_json(compare_run_dir / "larger_labeled_split_v2_summary.json")
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM post-symmetric next-step bootstrap validation",
