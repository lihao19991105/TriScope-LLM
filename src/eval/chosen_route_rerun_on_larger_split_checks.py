"""Validation helpers for chosen-route rerun on larger split artifacts."""
...
def validate_chosen_route_rerun_on_larger_split(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    plan_path = run_dir / "chosen_route_rerun_plan.json"
    readiness_path = run_dir / "chosen_route_rerun_readiness_summary.json"
    dataset_path = run_dir / "expanded_more_natural_dataset.jsonl"
    summary_path = run_dir / "expanded_more_natural_summary.json"
    predictions_path = run_dir / "expanded_more_natural_logistic_predictions.jsonl"
    logistic_summary_path = run_dir / "expanded_more_natural_logistic_summary.json"
    run_summary_path = run_dir / "chosen_route_rerun_run_summary.json"
    ...
    acceptance = {
        ...
        "field_checks": {
            ...
            "chosen_route_is_B": plan.get("chosen_route") == "B",
    ...
    if compare_run_dir is not None:
        compare_summary = load_json(compare_run_dir / "expanded_more_natural_summary.json")
        compare_logistic = load_json(compare_run_dir / "expanded_more_natural_logistic_summary.json")
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM chosen route rerun on larger split validation",
