"""Dry-run the refined-fusion-aware next-axis real-experiment matrix v4."""
...
SCHEMA_VERSION = "triscopellm/next-axis-real-experiment-matrix-dry-run/v1"
...
def build_next_axis_real_experiment_matrix_dry_run(
    ...
    if matrix_definition.get("summary_status") != "PASS":
        raise ValueError("Next-axis matrix definition must be PASS before dry-run.")
    if matrix_bootstrap_summary.get("summary_status") != "PASS":
        raise ValueError("Next-axis matrix bootstrap summary must be PASS before dry-run.")
    ...
    for path in [*required_matrix_inputs, *required_v6_inputs]:
        if not path.is_file():
            raise ValueError(f"Next-axis matrix dry-run input not found: `{path}`.")

    materialized_execution_dir = output_dir / "materialized_next_axis_matrix_inputs"
    ...
    plan = {
        ...
        "dry_run_mode": "fusion_refinement_aware_matrix_simulation",
        "seed": seed,
    ...
    contract = {
        ...
        "route_role_map": {
            ...
            "fusion_cell_refined": "explicit refined-style fusion cell intended to sharpen candidate semantics beyond the inherited summary baseline",
        },
    }
    readiness = {
        "summary_status": "PASS",
        ...
        "notes": [
            ...
            "fusion_cell_refined is the refined explicit fusion cell intended to sharpen candidate semantics.",
        ],
    }
    write_json(output_dir / "next_axis_matrix_dry_run_plan.json", plan)
    write_json(output_dir / "next_axis_matrix_execution_contract.json", contract)
    write_json(output_dir / "next_axis_matrix_readiness_summary.json", readiness)
    ...
    registry = {
        ...
        "matrix_name": matrix_bootstrap_summary["matrix_name"],
        "artifacts": {
            "plan": str((output_dir / "next_axis_matrix_dry_run_plan.json").resolve()),
            "contract": str((output_dir / "next_axis_matrix_execution_contract.json").resolve()),
            "readiness": str((output_dir / "next_axis_matrix_readiness_summary.json").resolve()),
            "module_status": str((output_dir / "next_axis_matrix_module_status.json").resolve()),
            "cell_status": str((output_dir / "next_axis_matrix_cell_status.json").resolve()),
        },
    }
    summary = {
        "summary_status": "PASS",
    ...
    write_json(output_dir / "next_axis_matrix_module_status.json", module_status)
    write_json(output_dir / "next_axis_matrix_cell_status.json", cell_status)
    write_json(output_dir / "next_axis_matrix_dry_run_registry.json", registry)
    write_json(output_dir / "next_axis_matrix_dry_run_summary.json", summary)
    write_jsonl(output_dir / "next_axis_matrix_dry_run_preview.jsonl", preview_rows)
    (output_dir / "next_axis_matrix_dry_run.log").write_text(
        "next-axis matrix dry-run complete\n"
    ...
    return {
        ...
        "output_paths": {
            ...
            "contract": str((output_dir / "next_axis_matrix_execution_contract.json").resolve()),
            "readiness": str((output_dir / "next_axis_matrix_readiness_summary.json").resolve()),
            "summary": str((output_dir / "next_axis_matrix_dry_run_summary.json").resolve()),
            "registry": str((output_dir / "next_axis_matrix_dry_run_registry.json").resolve()),
        },
    }
