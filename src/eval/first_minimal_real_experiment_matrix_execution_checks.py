"""Validation helpers for first minimal real-experiment matrix execution."""
...
def validate_first_minimal_real_experiment_matrix_execution(
    run_dir: Path,
    compare_run_dir: Path,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    run_summary = load_json(run_dir / "first_matrix_execution_run_summary.json")
    registry = load_json(run_dir / "first_matrix_execution_registry.json")
    metrics = load_json(run_dir / "first_matrix_execution_metrics.json")
    preview_path = run_dir / "first_matrix_execution_preview.jsonl"
    compare_metrics = load_json(compare_run_dir / "first_matrix_execution_metrics.json")
