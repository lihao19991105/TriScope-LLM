"""Time-separated replay confirmation for route_c stable baseline (stage 144)."""

from __future__ import annotations

from datetime import datetime, timezone
from pathlib import Path
from statistics import mean
from typing import Any

from src.eval.model_axis_1p5b_route_c_refined_execution import load_json, load_jsonl, write_json, write_jsonl
from src.eval.rerun_route_c_on_labeled_split_v6 import count_jsonl_rows, run_route_c_v6


SCHEMA_VERSION = "triscopellm/route-c-stable-baseline-time-separated-replay/v1"


def _required_file(path: Path) -> Path:
    if not path.is_file():
        raise ValueError(f"Required file not found: `{path}`")
    return path


def _label_set_from_class_balance(class_balance: dict[str, Any] | None) -> list[int]:
    if not isinstance(class_balance, dict):
        return []
    label_set: list[int] = []
    if int(class_balance.get("label_0", 0) or 0) > 0:
        label_set.append(0)
    if int(class_balance.get("label_1", 0) or 0) > 0:
        label_set.append(1)
    return label_set


def _metric_stats(values: list[float]) -> dict[str, float | None]:
    if not values:
        return {"min": None, "max": None, "mean": None}
    return {"min": min(values), "max": max(values), "mean": mean(values)}


def _rows_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    run_count = len(rows)
    gate_pass_flags = [str(row.get("gate_status")) == "PASS" for row in rows]
    dual_class_flags = [list(row.get("execution_label_set") or []) == [0, 1] for row in rows]
    consistency_flags = [bool(row.get("consistency_restored")) for row in rows]

    parsed_values = [float(row.get("parsed_option_count") or 0.0) for row in rows]
    missing_values = [float(row.get("missing_option_ratio") or 0.0) for row in rows]
    punct_values = [float(row.get("punct_only_ratio") or 0.0) for row in rows]

    regressed_runs = [
        str(row.get("replay_id"))
        for row in rows
        if str(row.get("gate_status")) != "PASS" or list(row.get("execution_label_set") or []) != [0, 1]
    ]

    return {
        "run_count": run_count,
        "gate_pass_rate": float(sum(gate_pass_flags)) / float(run_count) if run_count > 0 else 0.0,
        "dual_class_restoration_rate": float(sum(dual_class_flags)) / float(run_count) if run_count > 0 else 0.0,
        "consistency_restored_rate": float(sum(consistency_flags)) / float(run_count) if run_count > 0 else 0.0,
        "any_regression_to_blocked": any(not flag for flag in gate_pass_flags),
        "any_regression_to_single_class": any(not flag for flag in dual_class_flags),
        "regressed_runs": regressed_runs,
        "parsed_option_count_stats": _metric_stats(parsed_values),
        "missing_option_ratio_stats": _metric_stats(missing_values),
        "punct_only_ratio_stats": _metric_stats(punct_values),
    }


def _extract_gate_metrics(run_dir: Path) -> dict[str, Any]:
    gate_result_path = run_dir / "route_c_v6_label_health_gate_result.json"
    gate_result = load_json(gate_result_path) if gate_result_path.is_file() else {}

    dataset_dir = run_dir / "route_c_v6_dataset_dir"
    parse_compare_path = dataset_dir / "benchmark_truth_leaning_label_parse_compare.json"
    parse_compare = load_json(parse_compare_path) if parse_compare_path.is_file() else {}

    class_balance = gate_result.get("class_balance")
    execution_label_set = _label_set_from_class_balance(class_balance)
    normalized_dist = parse_compare.get("normalized", {}).get("failure_category_distribution", {})

    return {
        "gate_status": gate_result.get("gate_status"),
        "blocked_reason": gate_result.get("blocked_reason"),
        "parsed_option_count": gate_result.get("parsed_option_count"),
        "missing_option_ratio": gate_result.get("missing_option_ratio"),
        "punct_only_ratio": gate_result.get("punct_only_ratio"),
        "class_balance": class_balance,
        "execution_label_set": execution_label_set,
        "failure_category_distribution": normalized_dist,
        "option_parse_mode": gate_result.get("option_parse_mode"),
        "option_normalization_mode": gate_result.get("option_normalization_mode"),
    }


def _validate_frozen_against_current(
    frozen_settings: dict[str, Any],
    materialized_inputs_dir: Path,
    precheck_label_set: list[int],
) -> None:
    expected_input_root = str(materialized_inputs_dir.resolve())
    if str(frozen_settings.get("input_root")) != expected_input_root:
        raise ValueError(
            "Frozen input_root mismatch: "
            f"expected `{expected_input_root}`, got `{frozen_settings.get('input_root')}`"
        )

    current_budgets = {
        "reasoning_budget": count_jsonl_rows(materialized_inputs_dir / "reasoning_query_contracts.jsonl"),
        "confidence_budget": count_jsonl_rows(materialized_inputs_dir / "confidence_query_contracts.jsonl"),
        "illumination_budget": count_jsonl_rows(materialized_inputs_dir / "illumination_query_contracts.jsonl"),
        "labeled_illumination_budget": count_jsonl_rows(
            materialized_inputs_dir / "labeled_illumination_query_contracts.jsonl"
        ),
    }
    for key, value in current_budgets.items():
        if int(frozen_settings.get(key, -1)) != int(value):
            raise ValueError(
                f"Frozen `{key}` mismatch: expected {value}, got {frozen_settings.get(key)}"
            )

    if list(frozen_settings.get("precheck_label_set_reference") or []) != precheck_label_set:
        raise ValueError(
            "Frozen precheck label set mismatch: "
            f"expected {precheck_label_set}, got {frozen_settings.get('precheck_label_set_reference')}"
        )


def _compute_time_separation(
    stage_143_summary_path: Path,
    stage_143_verdict_path: Path,
    min_time_separation_minutes: float,
) -> dict[str, Any]:
    stage_143_latest_ts = max(stage_143_summary_path.stat().st_mtime, stage_143_verdict_path.stat().st_mtime)
    replay_start_ts = datetime.now(timezone.utc).timestamp()
    elapsed_minutes = (replay_start_ts - stage_143_latest_ts) / 60.0

    stage_143_latest_utc = datetime.fromtimestamp(stage_143_latest_ts, tz=timezone.utc)
    replay_start_utc = datetime.fromtimestamp(replay_start_ts, tz=timezone.utc)
    protocol_valid = bool(elapsed_minutes >= float(min_time_separation_minutes))

    return {
        "stage_143_latest_completion_time_utc": stage_143_latest_utc.isoformat(),
        "replay_start_time_utc": replay_start_utc.isoformat(),
        "elapsed_minutes_since_stage_143": elapsed_minutes,
        "minimum_required_minutes": float(min_time_separation_minutes),
        "protocol_valid": protocol_valid,
    }


def build_route_c_stable_baseline_time_separated_replay(
    route_c_anchor_followup_v2_dir: Path,
    route_c_anchor_execution_recheck_dir: Path,
    stage_143_recheck_dir: Path,
    stage_143_analysis_dir: Path,
    models_config_path: Path,
    reasoning_config_path: Path,
    confidence_config_path: Path,
    illumination_config_path: Path,
    reasoning_prompt_dir: Path,
    confidence_prompt_dir: Path,
    illumination_prompt_dir: Path,
    output_dir: Path,
    replay_count: int = 3,
    min_time_separation_minutes: float = 10.0,
) -> dict[str, Any]:
    if replay_count < 3:
        raise ValueError("replay_count must be >= 3 for minimal time-separated confirmation.")

    output_dir.mkdir(parents=True, exist_ok=True)

    materialized_inputs_dir = route_c_anchor_execution_recheck_dir / "materialized_route_c_anchor_execution_inputs"
    if not materialized_inputs_dir.is_dir():
        raise ValueError(f"Controlled input dir not found: `{materialized_inputs_dir}`")

    stage_143_frozen_path = _required_file(
        stage_143_recheck_dir / "route_c_label_output_normalization_stability_extension_frozen_settings.json"
    )
    stage_143_summary_path = _required_file(
        stage_143_recheck_dir / "route_c_label_output_normalization_stability_extension_recheck_summary.json"
    )
    stage_143_details_path = _required_file(
        stage_143_recheck_dir / "route_c_label_output_normalization_stability_extension_recheck_details.jsonl"
    )
    stage_143_compare_path = _required_file(
        stage_143_analysis_dir / "route_c_label_output_normalization_stability_extension_compare.json"
    )
    stage_143_verdict_path = _required_file(
        stage_143_analysis_dir / "route_c_label_output_normalization_stability_extension_verdict.json"
    )

    stage_143_frozen = load_json(stage_143_frozen_path)
    stage_143_summary = load_json(stage_143_summary_path)
    stage_143_compare = load_json(stage_143_compare_path)
    stage_143_details = load_jsonl(stage_143_details_path)

    stage_142_details_path = Path(
        str(stage_143_frozen.get("inherits_from", {}).get("details_path") or "")
    )
    stage_142_details = load_jsonl(_required_file(stage_142_details_path))

    precheck = load_json(_required_file(route_c_anchor_followup_v2_dir / "route_c_anchor_followup_v2_precheck.json"))
    precheck_label_set = _label_set_from_class_balance(precheck.get("class_balance"))

    frozen_settings = dict(stage_143_frozen.get("frozen_settings") or {})
    must_match_exactly = list(stage_143_frozen.get("must_match_exactly") or [])
    if not frozen_settings:
        raise ValueError("Stage-143 frozen settings payload is empty.")

    _validate_frozen_against_current(
        frozen_settings=frozen_settings,
        materialized_inputs_dir=materialized_inputs_dir,
        precheck_label_set=precheck_label_set,
    )

    time_separation = _compute_time_separation(
        stage_143_summary_path=stage_143_summary_path,
        stage_143_verdict_path=stage_143_verdict_path,
        min_time_separation_minutes=min_time_separation_minutes,
    )
    if not bool(time_separation["protocol_valid"]):
        raise ValueError(
            "Time-separated replay protocol not satisfied: "
            f"elapsed_minutes={time_separation['elapsed_minutes_since_stage_143']:.2f}, "
            f"required_minutes={time_separation['minimum_required_minutes']:.2f}."
        )

    seed = int(frozen_settings["seed"])
    label_threshold = float(frozen_settings["label_threshold"])
    label_parse_mode = str(frozen_settings["label_parse_mode"])
    label_normalization_mode = str(frozen_settings["label_normalization_mode"])
    model_profile_name = str(frozen_settings["model_profile_name"])
    parser_instrumentation = bool(frozen_settings["label_parser_instrumentation"])

    replay_rows: list[dict[str, Any]] = []
    for index in range(replay_count):
        replay_id = f"time_replay_{index + 1:02d}"
        run_dir = output_dir / "runs" / replay_id
        execution_error: str | None = None
        try:
            run_route_c_v6(
                models_config_path=models_config_path,
                reasoning_config_path=reasoning_config_path,
                confidence_config_path=confidence_config_path,
                illumination_config_path=illumination_config_path,
                reasoning_prompt_dir=reasoning_prompt_dir,
                confidence_prompt_dir=confidence_prompt_dir,
                illumination_prompt_dir=illumination_prompt_dir,
                v6_inputs_dir=materialized_inputs_dir,
                output_dir=run_dir,
                seed=seed,
                label_threshold=label_threshold,
                model_profile_name=model_profile_name,
                label_parse_mode=label_parse_mode,
                label_normalization_mode=label_normalization_mode,
                label_parser_instrumentation=parser_instrumentation,
            )
        except Exception as exc:
            execution_error = str(exc)

        gate = _extract_gate_metrics(run_dir)
        gate_status = str(gate.get("gate_status") or "UNKNOWN")
        execution_label_set = list(gate.get("execution_label_set") or [])
        consistency_restored = bool(gate_status == "PASS" and execution_label_set == precheck_label_set)

        replay_rows.append(
            {
                "schema_version": SCHEMA_VERSION,
                "replay_id": replay_id,
                "replay_tag": "144_time_separated_replay",
                "run_dir": str(run_dir.resolve()),
                "run_pass": bool(gate_status == "PASS" and execution_error is None),
                "gate_status": gate_status,
                "blocked_reason": gate.get("blocked_reason"),
                "consistency_restored": consistency_restored,
                "precheck_label_set": precheck_label_set,
                "execution_label_set": execution_label_set,
                "dual_class_restored": bool(execution_label_set == [0, 1]),
                "parsed_option_count": gate.get("parsed_option_count"),
                "missing_option_ratio": gate.get("missing_option_ratio"),
                "punct_only_ratio": gate.get("punct_only_ratio"),
                "class_balance": gate.get("class_balance"),
                "failure_category_distribution": gate.get("failure_category_distribution"),
                "option_parse_mode": gate.get("option_parse_mode"),
                "option_normalization_mode": gate.get("option_normalization_mode"),
                "execution_error": execution_error,
            }
        )

    replay_metrics = _rows_metrics(replay_rows)
    cumulative_rows = stage_142_details + stage_143_details + replay_rows
    cumulative_metrics = _rows_metrics(cumulative_rows)

    prev_cumulative = dict(stage_143_summary.get("cumulative_142_143") or {})
    prev_parsed_min = float(prev_cumulative.get("parsed_option_count_stats", {}).get("min") or 0.0)
    prev_missing_max = float(prev_cumulative.get("missing_option_ratio_stats", {}).get("max") or 0.0)
    prev_punct_max = float(prev_cumulative.get("punct_only_ratio_stats", {}).get("max") or 0.0)

    replay_parsed_min = float(replay_metrics.get("parsed_option_count_stats", {}).get("min") or 0.0)
    replay_missing_max = float(replay_metrics.get("missing_option_ratio_stats", {}).get("max") or 0.0)
    replay_punct_max = float(replay_metrics.get("punct_only_ratio_stats", {}).get("max") or 0.0)

    parseability_drift_vs_143 = bool(
        replay_parsed_min < prev_parsed_min
        or replay_missing_max > prev_missing_max
        or replay_punct_max > prev_punct_max
    )

    frozen_payload = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "inherits_from": {
            "stage": "143-route-c-output-normalization-stability-extension-recheck",
            "frozen_settings_path": str(stage_143_frozen_path.resolve()),
            "summary_path": str(stage_143_summary_path.resolve()),
            "details_path": str(stage_143_details_path.resolve()),
            "compare_path": str(stage_143_compare_path.resolve()),
            "verdict_path": str(stage_143_verdict_path.resolve()),
            "previous_cumulative_rerun_count": int(prev_cumulative.get("run_count", len(stage_142_details) + len(stage_143_details))),
        },
        "frozen_settings": frozen_settings,
        "must_match_exactly": must_match_exactly,
        "allowed_replay_tag_only_fields": [
            "replay_id",
            "run_dir",
            "replay_tag",
            "timestamp_fields",
            "report_text",
        ],
        "time_separation_protocol": {
            "definition": "replay runs started after stage-143 completion with frozen settings unchanged",
            "minimum_required_minutes": float(min_time_separation_minutes),
            "time_separation": time_separation,
        },
        "replay_verdict_policy": {
            "replay_confirmed": {
                "time_separation_protocol_valid": True,
                "all_replay_runs_pass": True,
                "no_replay_regression": True,
                "parseability_stable_bounds": {
                    "parsed_option_count_min": 5,
                    "missing_option_ratio_max": 0.16666666666666666,
                    "punct_only_ratio_max": 0.0,
                },
                "no_cumulative_regression": True,
            },
            "replay_partially_confirmed": {
                "core_restoration_holds_with_minor_uncertainty": True,
            },
            "replay_not_confirmed": {
                "any_regression_to_blocked": True,
                "any_regression_to_single_class": True,
                "consistency_instability": True,
                "material_parseability_regression": True,
                "time_separation_protocol_invalid": True,
            },
        },
    }

    summary = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "replay_count": replay_count,
        "time_separation": time_separation,
        "replay_only": replay_metrics,
        "vs_stage_143_cumulative": {
            "gate_pass_rate_delta": replay_metrics["gate_pass_rate"] - float(prev_cumulative.get("gate_pass_rate", 0.0) or 0.0),
            "dual_class_restoration_rate_delta": replay_metrics["dual_class_restoration_rate"]
            - float(prev_cumulative.get("dual_class_restoration_rate", 0.0) or 0.0),
            "consistency_restored_rate_delta": replay_metrics["consistency_restored_rate"]
            - float(prev_cumulative.get("consistency_restored_rate", 0.0) or 0.0),
            "parseability_drift_vs_143": parseability_drift_vs_143,
            "parsed_option_count_min_delta": replay_parsed_min - prev_parsed_min,
            "missing_option_ratio_max_delta": replay_missing_max - prev_missing_max,
            "punct_only_ratio_max_delta": replay_punct_max - prev_punct_max,
            "parser_or_normalization_drift_detected": parseability_drift_vs_143,
        },
        "cumulative_142_143_144": cumulative_metrics,
        "five_stage_compare_source": {
            "stage_140_to_143_compare_path": str(stage_143_compare_path.resolve()),
            "stage_143_summary_path": str(stage_143_summary_path.resolve()),
        },
        "constraints_kept": {
            "benchmark_truth_semantics_unchanged": True,
            "parser_logic_unchanged": True,
            "normalization_logic_unchanged": True,
            "gate_logic_unchanged": True,
            "execution_path_unchanged": True,
            "no_budget_expansion": True,
            "no_model_axis_expansion": True,
            "no_prompt_family_expansion": True,
            "no_cherry_pick": True,
            "all_replay_runs_retained": True,
        },
    }

    write_json(
        output_dir / "route_c_stable_baseline_time_separated_replay_frozen_settings.json",
        frozen_payload,
    )
    write_json(
        output_dir / "route_c_stable_baseline_time_separated_replay_summary.json",
        summary,
    )
    write_jsonl(
        output_dir / "route_c_stable_baseline_time_separated_replay_details.jsonl",
        replay_rows,
    )

    return {
        "summary": summary,
        "output_paths": {
            "frozen_settings": str(
                (
                    output_dir
                    / "route_c_stable_baseline_time_separated_replay_frozen_settings.json"
                ).resolve()
            ),
            "summary": str(
                (
                    output_dir
                    / "route_c_stable_baseline_time_separated_replay_summary.json"
                ).resolve()
            ),
            "details": str(
                (
                    output_dir
                    / "route_c_stable_baseline_time_separated_replay_details.jsonl"
                ).resolve()
            ),
        },
    }
