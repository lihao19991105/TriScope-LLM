"""Minimal continuous-execution validation for route_c frozen semantic boundary behavior."""
...
SCHEMA_VERSION = "triscopellm/route-c-frozen-semantic-minimal-continuous-execution-validation/v1"
...
def _minimal_continuous_suite(
	...
	normal_anchor_two = by_sample["csqa-pilot-021__control"]
	...
	cases: list[dict[str, Any]] = [
		{
			"case_id": "continuous_exec::w01::recoverable::code_fence",
			"path_group": "recoverable_continuous_execution",
			"source_type": "controlled-format-variant-from-real",
			"is_real_execution_input": False,
			"is_continuous_execution_input": True,
			"continuous_window_id": "window_01",
			...
			"case_note": "window_01 recoverable code-fence wrapper anchored to real execution parser-reachable response",
		},
		{
			"case_id": "continuous_exec::w01::normal::csqa-pilot-002__control",
			"path_group": "normal_continuous_execution_guardrail",
			"source_type": "real",
			"is_real_execution_input": True,
			"is_continuous_execution_input": True,
			"continuous_window_id": "window_01",
			"continuous_position": 2,
			"real_execution_anchor_sample_id": "csqa-pilot-002__control",
			"raw_response": str(recoverable_anchor["response_text"]),
			"query_answer_key": _query_answer_key(recoverable_anchor),
			...
			"stage_151_reference_category": stage_151_reference_by_case["real_exec::real_normal::csqa-pilot-002__control"][
				"current_category"
			],
			"code_fence_like": False,
			"case_note": "window_01 normal guardrail case using real parser-reachable response",
		},
		{
			"case_id": "continuous_exec::w01::nonrecoverable::csqa-pilot-021__targeted",
			"path_group": "nonrecoverable_continuous_execution_guardrail",
			"source_type": "real",
			"is_real_execution_input": True,
			"is_continuous_execution_input": True,
			"continuous_window_id": "window_01",
			"continuous_position": 3,
			...
			"code_fence_like": False,
			"case_note": "window_01 real nonrecoverable guardrail case",
		},
		{
			"case_id": "continuous_exec::w02::recoverable::code_fence_lang_tag",
			"path_group": "recoverable_continuous_execution",
			"source_type": "controlled-format-variant-from-real",
			"is_real_execution_input": False,
			"is_continuous_execution_input": True,
			"continuous_window_id": "window_02",
			"continuous_position": 1,
			"real_execution_anchor_sample_id": "csqa-pilot-002__control",
			"raw_response": f"```text\n{recoverable_raw}\n```",
			"query_answer_key": _query_answer_key(recoverable_anchor),
			...
			"code_fence_like": True,
			"case_note": "window_02 language-tagged recoverable code-fence wrapper anchored to same real response",
		},
		{
			"case_id": "continuous_exec::w02::normal::csqa-pilot-021__control",
			"path_group": "normal_continuous_execution_guardrail",
			"source_type": "real",
			"is_real_execution_input": True,
			"is_continuous_execution_input": True,
			...
			"stage_151_reference_category": stage_151_reference_by_case["real_exec::real_normal::csqa-pilot-021__control"][
				"current_category"
			],
			"code_fence_like": False,
			"case_note": "window_02 second normal guardrail case using real parser-reachable response",
		},
		{
			"case_id": "continuous_exec::w02::nonrecoverable::code_fence_contract_broken",
			"path_group": "nonrecoverable_continuous_execution_guardrail",
			"source_type": "controlled-format-variant-from-real",
			"is_real_execution_input": False,
			"is_continuous_execution_input": True,
			...
			"code_fence_like": True,
			"case_note": "window_02 near-boundary nonrecoverable code-fence case anchored to real nonrecoverable response",
		},
	]
	return cases
...
def _materialize_continuous_execution_inputs(
	cases: list[dict[str, Any]],
	real_raw_rows: list[dict[str, Any]],
		...
		contract_metadata.update(
			{
				...
				"contract_variant": f"continuous_exec_validation_{index:02d}",
				"query_answer_key": str(case["query_answer_key"]),
				"label_source": "minimal_continuous_execution_validation",
				"label_scope": "benchmark_truth_leaning_supervision_proxy",
			}
		)
		metadata["contract_metadata"] = contract_metadata
		metadata["continuous_execution_validation_reference"] = {
			"case_id": case["case_id"],
			"path_group": case["path_group"],
			"source_type": case["source_type"],
			"continuous_window_id": case["continuous_window_id"],
			"continuous_position": int(case["continuous_position"]),
			"is_real_execution_input": bool(case["is_real_execution_input"]),
			"is_continuous_execution_input": bool(case["is_continuous_execution_input"]),
		}

		labeled_rows.append(
			{
				**template,
				"sample_id": f"continuous_exec_validation_{index:02d}__{case['case_id'].split('::')[-1]}",
	...
	dataset_input_path = execution_root_dir / "execution_inputs" / "minimal_continuous_execution_labeled_raw_results.jsonl"
	fusion_path = execution_root_dir / "execution_inputs" / "minimal_continuous_execution_fusion_dataset.jsonl"
	...
	manifest = {
		...
		"continuous_window_ids": sorted({str(case["continuous_window_id"]) for case in cases}),
	...
	write_json(execution_root_dir / "execution_inputs" / "continuous_execution_input_manifest.json", manifest)
	return manifest
...
def _evaluate_records(cases: list[dict[str, Any]], health_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
	...
	for index, case in enumerate(cases):
		...
		if case["path_group"] == "recoverable_continuous_execution" and current_handoff != "pass_formatted_to_parser":
			mismatch_type = "recoverable_continuous_regression"
		elif case["path_group"] == "normal_continuous_execution_guardrail" and current_handoff != "pass_raw_to_parser":
			mismatch_type = "normal_continuous_damage"
		elif (
			case["path_group"] == "nonrecoverable_continuous_execution_guardrail"
			and current_handoff != "degeneration_blocked"
		):
			mismatch_type = "nonrecoverable_continuous_leak"
...
def _window_summary(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
	...
	for row in records:
		by_window[str(row["continuous_window_id"])].append(row)

	output: list[dict[str, Any]] = []
	for window_id in sorted(by_window):
		rows = sorted(by_window[window_id], key=lambda item: int(item["continuous_position"]))
...
def _summary(
	...
	recoverable_records = [r for r in records if r["path_group"] == "recoverable_continuous_execution"]
	normal_records = [r for r in records if r["path_group"] == "normal_continuous_execution_guardrail"]
	nonrecoverable_records = [r for r in records if r["path_group"] == "nonrecoverable_continuous_execution_guardrail"]
	...
	focused_validation_answers = {
		"recoverable_boundary_stable_on_continuous_execution": int(
			mismatch_counts.get("recoverable_continuous_regression", 0)
		)
		== 0,
		"code_fence_like_recoverable_handoff_stable": int(mismatch_counts.get("recoverable_continuous_regression", 0)) == 0
		and all(r["current_handoff"] == "pass_formatted_to_parser" for r in code_fence_records),
		"normal_continuous_execution_cases_all_pass": int(mismatch_counts.get("normal_continuous_damage", 0)) == 0,
		"nonrecoverable_continuous_execution_cases_all_blocked": int(
			mismatch_counts.get("nonrecoverable_continuous_leak", 0)
		)
		== 0,
		"parser_gate_health_consistent_with_149_150_151": int(mismatch_counts.get("stage151_reference_drift", 0)) == 0
		and int(mismatch_counts.get("stage150_reference_drift", 0)) == 0
		...
		"impact_scope_limited_to_recoverable_boundary": int(mismatch_counts.get("normal_continuous_damage", 0)) == 0
		and int(mismatch_counts.get("nonrecoverable_continuous_leak", 0)) == 0,
	}
	...
	return {
		...
		"continuous_window_count": len(window_summary),
		"continuous_window_summary": window_summary,
		...
		"recoverable_continuous_regression_count": int(mismatch_counts.get("recoverable_continuous_regression", 0)),
		"normal_continuous_damage_count": int(mismatch_counts.get("normal_continuous_damage", 0)),
		"nonrecoverable_continuous_leak_count": int(mismatch_counts.get("nonrecoverable_continuous_leak", 0)),
		...
		"minimal_collateral_continuous_analysis": {
			"new_false_block_count": int(mismatch_counts.get("normal_continuous_damage", 0)),
			"new_leak_count": int(mismatch_counts.get("nonrecoverable_continuous_leak", 0)),
			"real_execution_correct_but_continuous_unstable": bool(
				int(mismatch_counts.get("stage151_reference_drift", 0)) > 0
			...
			"any_mismatch_detected": any(
				int(count) > 0 for key, count in mismatch_counts.items() if key != "match"
			),
		},
...
def _report(summary: dict[str, Any], records: list[dict[str, Any]]) -> str:
	lines: list[str] = []
	lines.append("# Route-C Frozen Semantic Minimal Continuous Execution Validation Report")
	lines.append("")
	lines.append("## Continuous-Execution Summary")
	lines.append(f"- recoverable_continuous_pass_rate: {summary['recoverable_continuous_pass_rate']}")
	lines.append(f"- code_fence_recoverable_pass_rate: {summary['code_fence_recoverable_pass_rate']}")
	lines.append(f"- normal_continuous_pass_rate: {summary['normal_continuous_pass_rate']}")
	lines.append(f"- nonrecoverable_continuous_block_rate: {summary['nonrecoverable_continuous_block_rate']}")
	...
	lines.append(f"- stage151_anchored_cases_match_frozen_reference: {summary['stage151_anchored_cases_match_frozen_reference']}")
	lines.append("")
	lines.append("## Continuous Window Consistency")
	for window in summary["continuous_window_summary"]:
		lines.append(
			"- "
	...
	lines.append(
		"- recoverable formatting boundary on continuous execution stable: "
		+ str(answers["recoverable_boundary_stable_on_continuous_execution"])
	)
	lines.append(
		"- code-fence-like recoverable handoff on continuous execution stable: "
		+ str(answers["code_fence_like_recoverable_handoff_stable"])
	)
	lines.append("- normal continuous-execution cases all pass: " + str(answers["normal_continuous_execution_cases_all_pass"]))
	lines.append(
		"- nonrecoverable continuous-execution cases all blocked: "
		+ str(answers["nonrecoverable_continuous_execution_cases_all_blocked"])
	)
	lines.append(
		"- parser/gate/health consistency with 149/150/151 kept: "
		+ str(answers["parser_gate_health_consistent_with_149_150_151"])
	)
	lines.append(
	...
	lines.append("")
	lines.append("## Minimal Collateral Continuous-Execution Analysis")
	collateral = summary["minimal_collateral_continuous_analysis"]
	...
	lines.append(
		"- real_execution_correct_but_continuous_unstable: "
		+ str(collateral["real_execution_correct_but_continuous_unstable"])
	)
	...
	for row in records:
		lines.append(
			"- "
			+ f"{row['case_id']} ({row['continuous_window_id']}#{row['continuous_position']}): "
	...
	lines.append(f"- stage_151_recoverable_real_execution_pass_rate_reference: {snapshots['stage_151'].get('recoverable_real_execution_pass_rate')}")
	lines.append(f"- stage_151_normal_real_execution_pass_rate_reference: {snapshots['stage_151'].get('normal_real_execution_pass_rate')}")
	lines.append(
...
def build_route_c_frozen_semantic_minimal_continuous_execution_validation(
	real_run_dir: Path,
	stage_148_dir: Path,
	stage_149_dir: Path,
	stage_150_dir: Path,
	...
	cases = _minimal_continuous_suite(
	...
	suite = {
		"summary_status": "PASS",
		"schema_version": SCHEMA_VERSION,
		"suite_name": "route_c_frozen_semantic_minimal_continuous_execution_v1",
		"case_count": len(cases),
		"continuous_window_ids": sorted({str(case["continuous_window_id"]) for case in cases}),
		"group_counts": dict(Counter(case["path_group"] for case in cases)),
		"source_type_counts": dict(Counter(case["source_type"] for case in cases)),
	...
	rules = {
		"summary_status": "PASS",
		"schema_version": SCHEMA_VERSION,
		"recoverable_continuous_execution_criteria": [
		...
		"normal_continuous_execution_criteria": [
		...
		"nonrecoverable_continuous_execution_criteria": [
		...
		"continuous_execution_consistency_criteria": [
			"parser/gate/label-health/handoff semantics remain coherent",
			"impact scope stays local to recoverable boundary",
			"no new path-level drift or gate/logistic/health anomaly in continuous windows",
		],
	}

	execution_root_dir = output_dir / "minimal_continuous_execution_root"
	input_manifest = _materialize_continuous_execution_inputs(
		cases=cases,
		real_raw_rows=real_raw_rows,
	...
	dataset_result = build_benchmark_truth_leaning_dataset(
		...
		/ "execution_inputs"
		/ "minimal_continuous_execution_fusion_dataset.jsonl",
		labeled_illumination_raw_results_path=execution_root_dir
		/ "execution_inputs"
		/ "minimal_continuous_execution_labeled_raw_results.jsonl",
		output_dir=dataset_dir,
		fusion_profile="route_c_frozen_semantic_minimal_continuous_execution",
		run_id="route_c_frozen_semantic_minimal_continuous_execution",
	...
	write_json(output_dir / "route_c_frozen_semantic_minimal_continuous_execution_suite.json", suite)
	write_json(output_dir / "route_c_frozen_semantic_minimal_continuous_execution_rules.json", rules)
	write_json(output_dir / "route_c_frozen_semantic_minimal_continuous_execution_summary.json", summary)
	write_jsonl(output_dir / "route_c_frozen_semantic_minimal_continuous_execution_details.jsonl", records)
	(output_dir / "route_c_frozen_semantic_minimal_continuous_execution_report.md").write_text(report, encoding="utf-8")
	...
	write_json(
		output_dir / "config_snapshot.json",
		{
			...
			"input_manifest_path": str(
				(execution_root_dir / "execution_inputs" / "continuous_execution_input_manifest.json").resolve()
			),
	...
	return {
		...
		"summary": summary,
		"output_paths": {
			"suite": str((output_dir / "route_c_frozen_semantic_minimal_continuous_execution_suite.json").resolve()),
			"rules": str((output_dir / "route_c_frozen_semantic_minimal_continuous_execution_rules.json").resolve()),
			"summary": str((output_dir / "route_c_frozen_semantic_minimal_continuous_execution_summary.json").resolve()),
			"details": str((output_dir / "route_c_frozen_semantic_minimal_continuous_execution_details.jsonl").resolve()),
			"report": str((output_dir / "route_c_frozen_semantic_minimal_continuous_execution_report.md").resolve()),
