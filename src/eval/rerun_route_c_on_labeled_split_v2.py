"""Rerun route C on the labeled split v2 substrate."""
...
SCHEMA_VERSION = "triscopellm/rerun-route-c-on-labeled-split-v2/v1"
...
def materialize_route_c_v2(
    ...
    if bootstrap_plan.get("chosen_next_step") != "prepare_larger_labeled_split_v2":
        raise ValueError("038 expects `chosen_next_step` to be `prepare_larger_labeled_split_v2`.")

    required_inputs = [
        v2_inputs_dir / "reasoning_query_contracts.jsonl",
        v2_inputs_dir / "confidence_query_contracts.jsonl",
        v2_inputs_dir / "illumination_query_contracts.jsonl",
        v2_inputs_dir / "labeled_illumination_query_contracts.jsonl",
        v2_inputs_dir / "csqa_reasoning_pilot_slice.jsonl",
    ]
    for path in required_inputs:
        if not path.is_file():
            raise ValueError(f"Route C v2 input not found: `{path}`.")

    materialized_inputs_dir = output_dir / "materialized_route_c_v2_inputs"
    ...
    plan = {
        ...
        "chosen_dataset_substrate": "larger_csqa_reasoning_pilot_local_v2",
        "selected_from": str(bootstrap_plan_path.resolve()),
        "success_criterion": [
            "materialize route C on labeled split v2",
            "produce a non-empty contract-level benchmark-truth-leaning dataset on v2",
            "run route C v2 logistic bootstrap",
        ],
        "known_risks": [
            "Still benchmark-truth-leaning proxy supervision, not benchmark ground truth.",
            "Still tied to the local curated v2 split and pilot_distilgpt2_hf.",
        ],
    }
    label_definition = {
        "schema_version": SCHEMA_VERSION,
        ...
        "row_expansion_rule": "each v2 base sample emits control/targeted contract rows through labeled illumination contracts",
        "label_limitations": [
    ...
    readiness_summary = {
        ...
        "chosen_route": "C",
        "ready_to_run": True,
        "v2_inputs_dir": str(v2_inputs_dir.resolve()),
        ...
        "expected_row_capacity": {
            "route_c_v2_alignment_rows": reasoning_rows,
            "route_c_v2_contract_rows": labeled_rows,
        },
        "notes": [
            "This rerun keeps route C's benchmark-truth-leaning supervision semantics while moving it onto labeled split v2.",
            "It extends route C from the 20-row larger split to the 30-row larger split v2.",
        ],
    }

    write_json(output_dir / "route_c_v2_plan.json", plan)
    write_json(output_dir / "route_c_v2_label_definition.json", label_definition)
    write_json(output_dir / "route_c_v2_readiness_summary.json", readiness_summary)
    write_json(
    ...
    (output_dir / "build.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route C on labeled split v2 materialization",
                "chosen_route=C",
                f"v2_inputs_dir={v2_inputs_dir.resolve()}",
    ...
    return {
        ...
        "output_paths": {
            "plan": str((output_dir / "route_c_v2_plan.json").resolve()),
            "label_definition": str((output_dir / "route_c_v2_label_definition.json").resolve()),
            "readiness_summary": str((output_dir / "route_c_v2_readiness_summary.json").resolve()),
            "materialized_inputs_dir": str(materialized_inputs_dir.resolve()),
...
def run_route_c_v2(
    ...
    reasoning_query_file = v2_inputs_dir / "reasoning_query_contracts.jsonl"
    confidence_query_file = v2_inputs_dir / "confidence_query_contracts.jsonl"
    illumination_query_file = v2_inputs_dir / "illumination_query_contracts.jsonl"
    labeled_illumination_query_file = v2_inputs_dir / "labeled_illumination_query_contracts.jsonl"

    v2_budget = count_jsonl_rows(reasoning_query_file)
    if v2_budget <= 0:
        raise ValueError("Route C v2 requires non-empty reasoning query contracts.")

    reasoning_dir = output_dir / "route_c_v2_reasoning"
    confidence_dir = output_dir / "route_c_v2_confidence"
    illumination_dir = output_dir / "route_c_v2_illumination"
    labeled_illumination_dir = output_dir / "route_c_v2_labeled_illumination"
    ...
    reasoning_probe = run_reasoning_probe(
        ...
        query_budget_override=v2_budget,
    ...
    reasoning_features = extract_reasoning_features(
        ...
        run_id="route_c_v2_reasoning",
    )
    write_json(
        reasoning_dir / "route_c_v2_reasoning_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
            "module_name": "reasoning",
            "target_budget": v2_budget,
    ...
    confidence_features = extract_confidence_features(
        ...
        run_id="route_c_v2_confidence",
        high_confidence_threshold=0.10,
    )
    write_json(
        confidence_dir / "route_c_v2_confidence_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    illumination_features = extract_illumination_features(
        ...
        run_id="route_c_v2_illumination",
    )
    write_json(
        illumination_dir / "route_c_v2_illumination_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    labeled_features = extract_illumination_features(
        ...
        run_id="route_c_v2_labeled_illumination",
    )
    write_json(
        labeled_illumination_dir / "route_c_v2_labeled_illumination_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    fusion_result = build_fusion_dataset(
        ...
        output_dir=output_dir / "route_c_v2_real_pilot_fusion",
        join_mode="inner",
    )
    ...
    dataset_result = build_benchmark_truth_leaning_dataset(
        ...
        output_dir=output_dir / "route_c_v2_dataset_dir",
        fusion_profile="benchmark_truth_leaning_real_pilot_v2",
        run_id="v2",
    )
    logistic_result = run_benchmark_truth_leaning_logistic(
        dataset_path=output_dir / "route_c_v2_dataset_dir" / "benchmark_truth_leaning_dataset.jsonl",
        output_dir=output_dir / "route_c_v2_run_dir",
        fusion_profile="benchmark_truth_leaning_real_pilot_v2",
        run_id="v2",
    ...
    copy_artifact(
        output_dir / "route_c_v2_dataset_dir" / "benchmark_truth_leaning_dataset.jsonl",
        output_dir / "route_c_v2_dataset.jsonl",
    )
    copy_artifact(
        output_dir / "route_c_v2_dataset_dir" / "benchmark_truth_leaning_dataset.csv",
        output_dir / "route_c_v2_dataset.csv",
    )
    copy_artifact(
        output_dir / "route_c_v2_dataset_dir" / "benchmark_truth_leaning_summary.json",
        output_dir / "route_c_v2_summary.json",
    )
    copy_artifact(
        output_dir / "route_c_v2_run_dir" / "benchmark_truth_leaning_logistic_predictions.jsonl",
        output_dir / "route_c_v2_logistic_predictions.jsonl",
    )
    copy_artifact(
        output_dir / "route_c_v2_run_dir" / "benchmark_truth_leaning_logistic_summary.json",
        output_dir / "route_c_v2_logistic_summary.json",
    )
    copy_artifact(
        output_dir / "route_c_v2_run_dir" / "benchmark_truth_leaning_model_metadata.json",
        output_dir / "route_c_v2_model_metadata.json",
    )
    ...
    run_summary = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "route_name": "benchmark_truth_leaning_supervision_proxy_v2",
        "route_c_v2_alignment": alignment_summary,
        "route_c_v2_summary": dataset_result["summary"],
        "route_c_v2_logistic_summary": logistic_result["summary"],
        "notes": [
            "This is route C rerun on the 30-row labeled split v2.",
    ...
    write_json(output_dir / "route_c_v2_run_summary.json", run_summary)
    (output_dir / "run.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route C on labeled split v2 rerun",
                f"v2_budget={v2_budget}",
                f"labeled_budget={labeled_budget}",
                f"alignment_rows={alignment_summary['num_rows']}",
                f"route_c_v2_rows={dataset_result['summary']['num_rows']}",
                f"route_c_v2_predictions={logistic_result['summary']['num_predictions']}",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    return {
        ...
        "output_paths": {
            "dataset_jsonl": str((output_dir / "route_c_v2_dataset.jsonl").resolve()),
            "dataset_csv": str((output_dir / "route_c_v2_dataset.csv").resolve()),
            "summary": str((output_dir / "route_c_v2_summary.json").resolve()),
            "predictions": str((output_dir / "route_c_v2_logistic_predictions.jsonl").resolve()),
            "logistic_summary": str((output_dir / "route_c_v2_logistic_summary.json").resolve()),
            "model_metadata": str((output_dir / "route_c_v2_model_metadata.json").resolve()),
            "run_summary": str((output_dir / "route_c_v2_run_summary.json").resolve()),
