"""Validation helpers for next real-experiment matrix dry-run."""
...
def validate_next_real_experiment_matrix_dry_run(
    run_dir: Path,
    compare_run_dir: Path,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    summary = load_json(run_dir / "next_matrix_dry_run_summary.json")
    registry = load_json(run_dir / "next_matrix_dry_run_registry.json")
    module_status = load_json(run_dir / "next_matrix_module_status.json")
    cell_status = load_json(run_dir / "next_matrix_cell_status.json")
    preview_path = run_dir / "next_matrix_dry_run_preview.jsonl"
    compare_summary = load_json(compare_run_dir / "next_matrix_dry_run_summary.json")
    ...
    acceptance = {
        ...
        "snapshot": {
            ...
            "num_passed_routes": len(summary.get("passed_routes", [])),
        },
    }
