"""Dry-run the refined-fusion-support-sweep-aware next-axis-after-v5 matrix v6."""
...
SCHEMA_VERSION = "triscopellm/next-axis-after-v5-matrix-dry-run/v1"
...
def build_next_axis_after_v5_matrix_dry_run(
    ...
    if matrix_definition.get("summary_status") != "PASS":
        raise ValueError("Next-axis-after-v5 matrix definition must be PASS before dry-run.")
    if matrix_bootstrap_summary.get("summary_status") != "PASS":
        raise ValueError("Next-axis-after-v5 matrix bootstrap summary must be PASS before dry-run.")
    ...
    for path in [*required_matrix_inputs, *required_v6_inputs]:
        if not path.is_file():
            raise ValueError(f"Next-axis-after-v5 matrix dry-run input not found: `{path}`.")

    materialized_execution_dir = output_dir / "materialized_next_axis_after_v5_matrix_inputs"
    ...
    plan = {
        ...
        "dry_run_mode": "fusion_refined_support_sweep_aware_matrix_simulation",
        "seed": seed,
    ...
    write_json(output_dir / "next_axis_after_v5_matrix_dry_run_plan.json", plan)
    write_json(output_dir / "next_axis_after_v5_matrix_execution_contract.json", contract)
    write_json(output_dir / "next_axis_after_v5_matrix_readiness_summary.json", readiness)
    ...
    registry = {
        ...
        "matrix_name": matrix_bootstrap_summary["matrix_name"],
        "artifacts": {
            "plan": str((output_dir / "next_axis_after_v5_matrix_dry_run_plan.json").resolve()),
            "contract": str((output_dir / "next_axis_after_v5_matrix_execution_contract.json").resolve()),
            "readiness": str((output_dir / "next_axis_after_v5_matrix_readiness_summary.json").resolve()),
            "module_status": str((output_dir / "next_axis_after_v5_matrix_module_status.json").resolve()),
            "cell_status": str((output_dir / "next_axis_after_v5_matrix_cell_status.json").resolve()),
        },
    }
    summary = {
        "summary_status": "PASS",
    ...
    write_json(output_dir / "next_axis_after_v5_matrix_module_status.json", module_status)
    write_json(output_dir / "next_axis_after_v5_matrix_cell_status.json", cell_status)
    write_json(output_dir / "next_axis_after_v5_matrix_dry_run_registry.json", registry)
    write_json(output_dir / "next_axis_after_v5_matrix_dry_run_summary.json", summary)
    write_jsonl(output_dir / "next_axis_after_v5_matrix_dry_run_preview.jsonl", preview_rows)
    (output_dir / "next_axis_after_v5_matrix_dry_run.log").write_text(
        "next-axis-after-v5 matrix dry-run complete\n"
    ...
    return {
        ...
        "output_paths": {
            ...
            "contract": str((output_dir / "next_axis_after_v5_matrix_execution_contract.json").resolve()),
            "readiness": str((output_dir / "next_axis_after_v5_matrix_readiness_summary.json").resolve()),
            "summary": str((output_dir / "next_axis_after_v5_matrix_dry_run_summary.json").resolve()),
            "registry": str((output_dir / "next_axis_after_v5_matrix_dry_run_registry.json").resolve()),
        },
    }
