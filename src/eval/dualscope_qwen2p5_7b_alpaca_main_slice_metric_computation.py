"""Compute bounded Qwen2.5-7B Alpaca main-slice metrics from real artifacts."""
...
SCHEMA_VERSION = "dualscope/qwen2p5-7b-alpaca-main-slice-metric-computation/v1"
TASK_ID = "dualscope-qwen2p5-7b-alpaca-main-slice-metric-computation"
FINAL_VALIDATED = "Qwen2.5-7B Alpaca main-slice metrics validated"
FINAL_PARTIAL = "Partially validated"
FINAL_NOT_VALIDATED = "Not validated"
NEXT_RESULT_PACKAGE = "dualscope-qwen2p5-7b-alpaca-main-slice-result-package"

RESPONSE_ROW_CANDIDATES = (
    "qwen2p5_7b_alpaca_main_slice_response_generation_rows.jsonl",
...
def _compute_detection_metrics(rows: list[dict[str, Any]], threshold: float) -> dict[str, Any]:
    ...
    return {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "metric_scope": "bounded_alpaca_main_slice_label_score_real_response_aligned",
        "row_count": len(rows),
...
def _compute_asr(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "metric_scope": "bounded_alpaca_main_slice_real_response_asr",
        "eligible_row_count": len(eligible),
...
def _compute_clean_utility(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "metric_scope": "bounded_alpaca_main_slice_real_response_clean_utility",
        "eligible_row_count": len(eligible),
...
def build_qwen2p5_7b_alpaca_main_slice_metric_computation(
    *,
    labeled_pairs: Path,
    response_dir: Path,
    score_dir: Path,
    ...
    if detection_pass and asr_pass and clean_utility_pass and py_compile["passed"]:
    ...
    recommendation = NEXT_RESULT_PACKAGE if final_verdict in {FINAL_VALIDATED, FINAL_PARTIAL} else "dualscope-qwen2p5-7b-alpaca-main-slice-metric-blocker-closure"
    ...
    verdict = {
        ...
        "final_verdict": final_verdict,
        "recommended_next_step": recommendation,
        "single_verdict_policy": "one_of_qwen2p5_7b_alpaca_main_slice_metrics_validated__partially_validated__not_validated",
    }
    ...
    write_json(output_dir / "blockers.json", {"summary_status": "PASS" if not blockers else "BLOCKED", "blockers": sorted(set(blockers))})
