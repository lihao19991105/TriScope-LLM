"""Rerun route C on the labeled split v5 substrate."""
...
SCHEMA_VERSION = "triscopellm/rerun-route-c-on-labeled-split-v5/v1"
...
def materialize_route_c_v5(
    ...
    if bootstrap_plan.get("chosen_next_step") != "prepare_larger_labeled_split_v5":
        raise ValueError("050 expects `chosen_next_step` to be `prepare_larger_labeled_split_v5`.")

    required_inputs = [
        v5_inputs_dir / "reasoning_query_contracts.jsonl",
        v5_inputs_dir / "confidence_query_contracts.jsonl",
        v5_inputs_dir / "illumination_query_contracts.jsonl",
        v5_inputs_dir / "labeled_illumination_query_contracts.jsonl",
        v5_inputs_dir / "csqa_reasoning_pilot_slice.jsonl",
    ]
    for path in required_inputs:
        if not path.is_file():
            raise ValueError(f"Route C v5 input not found: `{path}`.")

    materialized_inputs_dir = output_dir / "materialized_route_c_v5_inputs"
    ...
    plan = {
        ...
        "chosen_dataset_substrate": "larger_csqa_reasoning_pilot_local_v5",
        "selected_from": str(bootstrap_plan_path.resolve()),
        "success_criterion": [
            "materialize route C on labeled split v5",
            "produce a non-empty contract-level benchmark-truth-leaning dataset on v5",
            "run route C v5 logistic bootstrap",
        ],
        "known_risks": [
            "Still benchmark-truth-leaning proxy supervision, not benchmark ground truth.",
            "Still tied to the local curated v5 split and pilot_distilgpt2_hf.",
        ],
    }
    label_definition = {
        "schema_version": SCHEMA_VERSION,
        ...
        "row_expansion_rule": "each v5 base sample emits control/targeted contract rows through labeled illumination contracts",
        "label_limitations": [
    ...
    readiness_summary = {
        ...
        "chosen_route": "C",
        "ready_to_run": True,
        "v5_inputs_dir": str(v5_inputs_dir.resolve()),
        ...
        "expected_row_capacity": {
            "route_c_v5_alignment_rows": reasoning_rows,
            "route_c_v5_contract_rows": labeled_rows,
        },
        "notes": [
            "This rerun keeps route C's benchmark-truth-leaning supervision semantics while moving it onto labeled split v5.",
            "It extends route C from the 50-row v4 split to the 60-row labeled split v5.",
        ],
    }

    write_json(output_dir / "route_c_v5_plan.json", plan)
    write_json(output_dir / "route_c_v5_label_definition.json", label_definition)
    write_json(output_dir / "route_c_v5_readiness_summary.json", readiness_summary)
    write_json(
    ...
    (output_dir / "build.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route C on labeled split v5 materialization",
                "chosen_route=C",
                f"v5_inputs_dir={v5_inputs_dir.resolve()}",
    ...
    return {
        ...
        "output_paths": {
            "plan": str((output_dir / "route_c_v5_plan.json").resolve()),
            "label_definition": str((output_dir / "route_c_v5_label_definition.json").resolve()),
            "readiness_summary": str((output_dir / "route_c_v5_readiness_summary.json").resolve()),
            "materialized_inputs_dir": str(materialized_inputs_dir.resolve()),
...
def run_route_c_v5(
    ...
    reasoning_query_file = v5_inputs_dir / "reasoning_query_contracts.jsonl"
    confidence_query_file = v5_inputs_dir / "confidence_query_contracts.jsonl"
    illumination_query_file = v5_inputs_dir / "illumination_query_contracts.jsonl"
    labeled_illumination_query_file = v5_inputs_dir / "labeled_illumination_query_contracts.jsonl"

    v5_budget = count_jsonl_rows(reasoning_query_file)
    if v5_budget <= 0:
        raise ValueError("Route C v5 requires non-empty reasoning query contracts.")

    reasoning_dir = output_dir / "route_c_v5_reasoning"
    confidence_dir = output_dir / "route_c_v5_confidence"
    illumination_dir = output_dir / "route_c_v5_illumination"
    labeled_illumination_dir = output_dir / "route_c_v5_labeled_illumination"
    ...
    reasoning_probe = run_reasoning_probe(
        ...
        query_budget_override=v5_budget,
    ...
    reasoning_features = extract_reasoning_features(
        ...
        run_id="route_c_v5_reasoning",
    )
    write_json(
        reasoning_dir / "route_c_v5_reasoning_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
            "module_name": "reasoning",
            "target_budget": v5_budget,
    ...
    confidence_features = extract_confidence_features(
        ...
        run_id="route_c_v5_confidence",
        high_confidence_threshold=0.10,
    )
    write_json(
        confidence_dir / "route_c_v5_confidence_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    illumination_features = extract_illumination_features(
        ...
        run_id="route_c_v5_illumination",
    )
    write_json(
        illumination_dir / "route_c_v5_illumination_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    labeled_features = extract_illumination_features(
        ...
        run_id="route_c_v5_labeled_illumination",
    )
    write_json(
        labeled_illumination_dir / "route_c_v5_labeled_illumination_run_summary.json",
        {
            "summary_status": "PASS",
            "schema_version": SCHEMA_VERSION,
    ...
    fusion_result = build_fusion_dataset(
        ...
        output_dir=output_dir / "route_c_v5_real_pilot_fusion",
        join_mode="inner",
    )
    ...
    dataset_result = build_benchmark_truth_leaning_dataset(
        ...
        output_dir=output_dir / "route_c_v5_dataset_dir",
        fusion_profile="benchmark_truth_leaning_real_pilot_v5",
        run_id="v5",
    )
    logistic_result = run_benchmark_truth_leaning_logistic(
        dataset_path=output_dir / "route_c_v5_dataset_dir" / "benchmark_truth_leaning_dataset.jsonl",
        output_dir=output_dir / "route_c_v5_run_dir",
        fusion_profile="benchmark_truth_leaning_real_pilot_v5",
        run_id="v5",
    ...
    copy_artifact(
        output_dir / "route_c_v5_dataset_dir" / "benchmark_truth_leaning_dataset.jsonl",
        output_dir / "route_c_v5_dataset.jsonl",
    )
    copy_artifact(
        output_dir / "route_c_v5_dataset_dir" / "benchmark_truth_leaning_dataset.csv",
        output_dir / "route_c_v5_dataset.csv",
    )
    copy_artifact(
        output_dir / "route_c_v5_dataset_dir" / "benchmark_truth_leaning_summary.json",
        output_dir / "route_c_v5_summary.json",
    )
    copy_artifact(
        output_dir / "route_c_v5_run_dir" / "benchmark_truth_leaning_logistic_predictions.jsonl",
        output_dir / "route_c_v5_logistic_predictions.jsonl",
    )
    copy_artifact(
        output_dir / "route_c_v5_run_dir" / "benchmark_truth_leaning_logistic_summary.json",
        output_dir / "route_c_v5_logistic_summary.json",
    )
    copy_artifact(
        output_dir / "route_c_v5_run_dir" / "benchmark_truth_leaning_model_metadata.json",
        output_dir / "route_c_v5_model_metadata.json",
    )
    ...
    run_summary = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "route_name": "benchmark_truth_leaning_supervision_proxy_v5",
        "route_c_v5_alignment": alignment_summary,
        "route_c_v5_summary": dataset_result["summary"],
        "route_c_v5_logistic_summary": logistic_result["summary"],
        "notes": [
            "This is route C rerun on the 60-row labeled split v5.",
    ...
    write_json(output_dir / "route_c_v5_run_summary.json", run_summary)
    (output_dir / "run.log").write_text(
        "\n".join(
            [
                "TriScope-LLM route C on labeled split v5 rerun",
                f"v5_budget={v5_budget}",
                f"labeled_budget={labeled_budget}",
                f"alignment_rows={alignment_summary['num_rows']}",
                f"route_c_v5_rows={dataset_result['summary']['num_rows']}",
                f"route_c_v5_predictions={logistic_result['summary']['num_predictions']}",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    return {
        ...
        "output_paths": {
            "dataset_jsonl": str((output_dir / "route_c_v5_dataset.jsonl").resolve()),
            "dataset_csv": str((output_dir / "route_c_v5_dataset.csv").resolve()),
            "summary": str((output_dir / "route_c_v5_summary.json").resolve()),
            "predictions": str((output_dir / "route_c_v5_logistic_predictions.jsonl").resolve()),
            "logistic_summary": str((output_dir / "route_c_v5_logistic_summary.json").resolve()),
            "model_metadata": str((output_dir / "route_c_v5_model_metadata.json").resolve()),
            "run_summary": str((output_dir / "route_c_v5_run_summary.json").resolve()),
