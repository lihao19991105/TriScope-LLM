"""Compare supervision routes after route B and route C reruns on labeled split v2."""
...
SCHEMA_VERSION = "triscopellm/post-v2-symmetric-comparison/v1"
...
def build_post_v2_symmetric_comparison(
    ...
    route_c_v2_dir: Path,
    symmetric_comparison_dir: Path,
    post_symmetric_bootstrap_dir: Path,
    output_dir: Path,
) -> dict[str, Any]:
    ...
    prior_recommendation = load_json(
        symmetric_comparison_dir / "symmetric_rerun_next_step_recommendation.json"
    )
    v2_split_summary = load_json(post_symmetric_bootstrap_dir / "larger_labeled_split_v2_summary.json")
    ...
    comparison_rows = [
        ...
        },
        {
            ...
            "comparison_role": "v2_symmetric_candidate",
        },
        {
    ...
    progression_summary = {
        ...
        "v2_split_context": {
            "split_name": v2_split_summary["split_name"],
            "num_rows": v2_split_summary["num_rows"],
            "num_base_samples": v2_split_summary["num_base_samples"],
        },
        "progression": {
        ...
        "main_takeaway": "B and C now both exist on the same 30-row larger labeled split v2, so the main bottleneck shifts again from rerun asymmetry back to shared substrate growth.",
    }
    ...
    tradeoff_rows = [
        {
            "route_code": "B_v2",
            "route_name": "more_natural_supervision_proxy",
            "truth_anchor_level": route_b_v2_summary["label_naturalness_level"],
            "substrate_level": "larger_labeled_split_v2",
            "current_rows": route_b_v2_summary["num_rows"],
            "current_base_samples": route_b_v2_summary["num_base_samples"],
            "realism": "medium",
            "cost_to_expand_further": "medium",
            "next_ceiling": "shared_substrate_size",
            "why": "B now has symmetric substrate access at 30 rows, but still uses a weaker truth anchor than route C.",
        },
        {
            "route_code": "C_v2",
            "route_name": "benchmark_truth_leaning_supervision_proxy",
            "truth_anchor_level": route_c_v2_summary["label_naturalness_level"],
            "substrate_level": "larger_labeled_split_v2",
            "current_rows": route_c_v2_summary["num_rows"],
            "current_base_samples": route_c_v2_summary["num_base_samples"],
            "realism": "medium_high",
            "cost_to_expand_further": "medium",
            "next_ceiling": "shared_substrate_size",
            "why": "C preserves the stronger truth-leaning semantics, but both B and C again share the same substrate ceiling at v2.",
        },
        {
            "route_code": "substrate_v3",
            "route_name": "larger_labeled_split_v3",
            "truth_anchor_level": "shared_substrate_enabler",
            "substrate_level": "next_bootstrap",
            "current_rows": v2_split_summary["num_rows"],
            "current_base_samples": v2_split_summary["num_base_samples"],
            "realism": "enabler",
            "cost_to_expand_further": "medium",
            "next_ceiling": "local_curated_split_growth",
            "why": "A larger shared substrate would unlock the next symmetric comparison step for both B and C beyond the current 30-row ceiling.",
        },
    ]
    ...
    summary = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "v2_symmetric_ready": True,
        "main_question_answer": "After route B and route C both exist on labeled split v2, the next highest-value step is to prepare larger_labeled_split_v3, because both routes now share the same 30-row substrate ceiling.",
        "why": [
            "Route B v2 and route C v2 now both run on the same 30-row shared substrate.",
        ...
        "historical_context": {
            "036_recommendation": prior_recommendation.get("recommended_next_step"),
        },
    }
    ...
    recommendation = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "recommended_next_step": "prepare_larger_labeled_split_v3",
        "why_recommended": [
            "Route B and route C have both now been exercised on the same 30-row labeled split v2.",
            "Further route-specific reruns on the same v2 substrate would yield diminishing returns.",
            "A larger shared substrate v3 is the cleanest next enabler for both supervision routes.",
        ],
        "why_not_expand_B_first": [
            "B already has a true v2 rerun, so its main asymmetry problem has been removed.",
        ],
        "why_not_expand_C_first": [
            "C also already has a true v2 rerun, so another route-specific C-first step would be less informative than growing the shared substrate.",
        ],
        ...
        "minimum_success_standard": [
            "define larger split v3 contract",
            "materialize a larger shared labeled substrate beyond 30 rows",
            "prove route B / route C / fusion builders still attach cleanly",
        ],
    }

    write_json(output_dir / "v2_symmetric_comparison_summary.json", summary)
    write_csv(output_dir / "route_b_v2_vs_route_c_v2_comparison.csv", comparison_rows)
    write_json(output_dir / "supervision_progression_after_v2_rerun.json", progression_summary)
    write_csv(output_dir / "v2_tradeoff_matrix.csv", tradeoff_rows)
    write_json(output_dir / "v2_symmetric_next_step_recommendation.json", recommendation)
    (output_dir / "build.log").write_text(
        "\n".join(
            [
                "TriScope-LLM post-v2 symmetric comparison",
                f"route_b_v2_rows={route_b_v2_summary['num_rows']}",
                f"route_c_v2_rows={route_c_v2_summary['num_rows']}",
                "recommended_next_step=prepare_larger_labeled_split_v3",
            ]
        )
        + "\n",
    ...
    return {
        ...
        "output_paths": {
            "summary": str((output_dir / "v2_symmetric_comparison_summary.json").resolve()),
            "comparison_csv": str((output_dir / "route_b_v2_vs_route_c_v2_comparison.csv").resolve()),
            "progression": str((output_dir / "supervision_progression_after_v2_rerun.json").resolve()),
            "tradeoff_csv": str((output_dir / "v2_tradeoff_matrix.csv").resolve()),
            "recommendation": str((output_dir / "v2_symmetric_next_step_recommendation.json").resolve()),
