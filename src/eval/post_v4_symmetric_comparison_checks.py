"""Validation helpers for post-v4 symmetric comparison artifacts."""
...
def validate_post_v4_symmetric_comparison(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    summary_path = run_dir / "v4_symmetric_comparison_summary.json"
    comparison_csv_path = run_dir / "route_b_v4_vs_route_c_v4_comparison.csv"
    progression_path = run_dir / "supervision_progression_after_v4_rerun.json"
    tradeoff_csv_path = run_dir / "v4_tradeoff_matrix.csv"
    recommendation_path = run_dir / "v4_symmetric_next_step_recommendation.json"
    ...
    acceptance = {
        ...
        "field_checks": {
            ...
            "comparison_csv_has_rows": count_csv_rows(comparison_csv_path) >= 10,
        ...
        "snapshot": {
            ...
            "route_b_v4_rows": progression.get("progression", {}).get("route_b_v4_rows"),
            "route_c_v4_rows": progression.get("progression", {}).get("route_c_v4_rows"),
        },
    }
    ...
    if compare_run_dir is not None:
        compare_recommendation = load_json(compare_run_dir / "v4_symmetric_next_step_recommendation.json")
        compare_progression = load_json(compare_run_dir / "supervision_progression_after_v4_rerun.json")
        repeatability_summary = {
            ...
            "comparisons": [
                ...
                {
                    "field": "route_b_v4_rows",
                    "reference_value": progression.get("progression", {}).get("route_b_v4_rows"),
                    "candidate_value": compare_progression.get("progression", {}).get("route_b_v4_rows"),
                    "matches": progression.get("progression", {}).get("route_b_v4_rows")
                    == compare_progression.get("progression", {}).get("route_b_v4_rows"),
                },
                {
                    "field": "route_c_v4_rows",
                    "reference_value": progression.get("progression", {}).get("route_c_v4_rows"),
                    "candidate_value": compare_progression.get("progression", {}).get("route_c_v4_rows"),
                    "matches": progression.get("progression", {}).get("route_c_v4_rows")
                    == compare_progression.get("progression", {}).get("route_c_v4_rows"),
                },
            ],
        }
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM post-v4 symmetric comparison validation",
