"""Dry-run the refined-fusion-support-floor-stress-sweep-aware next-axis-after-v10 matrix v11."""
...
SCHEMA_VERSION = "triscopellm/next-axis-after-v10-matrix-dry-run/v1"
...
def build_next_axis_after_v10_matrix_dry_run(
    ...
    if matrix_definition.get("summary_status") != "PASS":
        raise ValueError("Next-axis-after-v10 matrix definition must be PASS before dry-run.")
    if matrix_bootstrap_summary.get("summary_status") != "PASS":
        raise ValueError("Next-axis-after-v10 matrix bootstrap summary must be PASS before dry-run.")
    ...
    required_v11_inputs = [
        v11_inputs_dir / "reasoning_query_contracts.jsonl",
        v11_inputs_dir / "confidence_query_contracts.jsonl",
        v11_inputs_dir / "illumination_query_contracts.jsonl",
        v11_inputs_dir / "labeled_illumination_query_contracts.jsonl",
    ]
    for path in [*required_matrix_inputs, *required_v11_inputs]:
        if not path.is_file():
            raise ValueError(f"Next-axis-after-v10 matrix dry-run input not found: `{path}`.")

    materialized_execution_dir = output_dir / "materialized_next_axis_after_v10_matrix_inputs"
    materialized_execution_dir.mkdir(parents=True, exist_ok=True)
    for src in [*required_matrix_inputs, *required_v11_inputs]:
    ...
    plan = {
        ...
        "dry_run_mode": "fusion_refined_support_ablation_floor_stress_sweep_aware_matrix_simulation",
        "seed": seed,
    ...
    contract = {
        ...
        "route_role_map": {
            ...
            "fusion_cell_refined_support_ablation_sweep": "explicit support-ablation sweep cell that perturbs that support-isolated floor to test whether any residual support-free refined evidence remains stable under further sweep",
            "fusion_cell_refined_support_ablation_floor_probe": "explicit floor-probe cell that exposes the swept support-isolated floor itself as a standalone executable signal",
            "fusion_cell_refined_support_ablation_floor_stress": "explicit floor-stress cell that directly stresses the exposed support floor to test whether its zero-floor conclusion remains invariant",
            "fusion_cell_refined_support_ablation_floor_stress_sweep": "explicit floor-stress-sweep cell that perturbs the already stressed explicit floor to test whether the direct-stress invariance conclusion still survives one more sweep",
        },
    }
    readiness = {
        "summary_status": "PASS",
        ...
        "notes": [
            ...
            "fusion_cell_refined_support_ablation_sweep perturbs that support-isolated floor to test whether any residual stability remains beyond the candidate floor.",
            "fusion_cell_refined_support_ablation_floor_probe turns that swept support-isolated floor into an explicit standalone probe cell.",
            "fusion_cell_refined_support_ablation_floor_stress directly stresses the explicit floor-probe result to test whether the floor remains invariant.",
            "fusion_cell_refined_support_ablation_floor_stress_sweep perturbs that already stressed explicit floor to check whether the direct-stress invariance result still survives one more sweep.",
        ],
    }
    write_json(output_dir / "next_axis_after_v10_matrix_dry_run_plan.json", plan)
    write_json(output_dir / "next_axis_after_v10_matrix_execution_contract.json", contract)
    ...
    registry = {
        ...
        "matrix_name": matrix_bootstrap_summary["matrix_name"],
        "artifacts": {
            "plan": str((output_dir / "next_axis_after_v10_matrix_dry_run_plan.json").resolve()),
            "contract": str((output_dir / "next_axis_after_v10_matrix_execution_contract.json").resolve()),
            "readiness": str((output_dir / "next_axis_after_v10_matrix_readiness_summary.json").resolve()),
            "module_status": str((output_dir / "next_axis_after_v10_matrix_module_status.json").resolve()),
            "cell_status": str((output_dir / "next_axis_after_v10_matrix_cell_status.json").resolve()),
        },
    }
    summary = {
        "summary_status": "PASS",
        ...
        "passed_fusion_refined_support_ablation_sweep_cells": [
            "dataset0_model0_fusion_cell_refined_support_ablation_sweep_explicit"
        ],
        "passed_fusion_refined_support_ablation_floor_probe_cells": [
            "dataset0_model0_fusion_cell_refined_support_ablation_floor_probe_explicit"
        ],
        "passed_fusion_refined_support_ablation_floor_stress_cells": [
            "dataset0_model0_fusion_cell_refined_support_ablation_floor_stress_explicit"
        ],
        "passed_fusion_refined_support_ablation_floor_stress_sweep_cells": [
            "dataset0_model0_fusion_cell_refined_support_ablation_floor_stress_sweep_explicit"
        ],
    }
    write_json(output_dir / "next_axis_after_v10_matrix_module_status.json", module_status)
    write_json(output_dir / "next_axis_after_v10_matrix_cell_status.json", cell_status)
    write_json(output_dir / "next_axis_after_v10_matrix_dry_run_registry.json", registry)
    write_json(output_dir / "next_axis_after_v10_matrix_dry_run_summary.json", summary)
    write_jsonl(output_dir / "next_axis_after_v10_matrix_dry_run_preview.jsonl", preview_rows)
    (output_dir / "next_axis_after_v10_matrix_dry_run.log").write_text(
        "next-axis-after-v10 matrix dry-run complete\n"
    ...
    return {
        ...
        "output_paths": {
            ...
            "contract": str((output_dir / "next_axis_after_v10_matrix_execution_contract.json").resolve()),
            "readiness": str((output_dir / "next_axis_after_v10_matrix_readiness_summary.json").resolve()),
            "summary": str((output_dir / "next_axis_after_v10_matrix_dry_run_summary.json").resolve()),
            "registry": str((output_dir / "next_axis_after_v10_matrix_dry_run_registry.json").resolve()),
        },
    }
