"""Validation helpers for post-v3 symmetric comparison artifacts."""
...
def validate_post_v3_symmetric_comparison(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    summary_path = run_dir / "v3_symmetric_comparison_summary.json"
    comparison_csv_path = run_dir / "route_b_v3_vs_route_c_v3_comparison.csv"
    progression_path = run_dir / "supervision_progression_after_v3_rerun.json"
    tradeoff_csv_path = run_dir / "v3_tradeoff_matrix.csv"
    recommendation_path = run_dir / "v3_symmetric_next_step_recommendation.json"
    ...
    acceptance = {
        ...
        "field_checks": {
            ...
            "comparison_csv_has_rows": count_csv_rows(comparison_csv_path) >= 8,
        ...
        "snapshot": {
            ...
            "route_b_v3_rows": progression.get("progression", {}).get("route_b_v3_rows"),
            "route_c_v3_rows": progression.get("progression", {}).get("route_c_v3_rows"),
        },
    }
    ...
    if compare_run_dir is not None:
        compare_recommendation = load_json(compare_run_dir / "v3_symmetric_next_step_recommendation.json")
        compare_progression = load_json(compare_run_dir / "supervision_progression_after_v3_rerun.json")
        repeatability_summary = {
            ...
            "comparisons": [
                ...
                {
                    "field": "route_b_v3_rows",
                    "reference_value": progression.get("progression", {}).get("route_b_v3_rows"),
                    "candidate_value": compare_progression.get("progression", {}).get("route_b_v3_rows"),
                    "matches": progression.get("progression", {}).get("route_b_v3_rows")
                    == compare_progression.get("progression", {}).get("route_b_v3_rows"),
                },
                {
                    "field": "route_c_v3_rows",
                    "reference_value": progression.get("progression", {}).get("route_c_v3_rows"),
                    "candidate_value": compare_progression.get("progression", {}).get("route_c_v3_rows"),
                    "matches": progression.get("progression", {}).get("route_c_v3_rows")
                    == compare_progression.get("progression", {}).get("route_c_v3_rows"),
                },
            ],
        }
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM post-v3 symmetric comparison validation",
