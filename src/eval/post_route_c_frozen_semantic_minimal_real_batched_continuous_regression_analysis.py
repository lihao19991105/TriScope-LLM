"""Post-analysis for route_c frozen semantic minimal real batched continuous regression."""
...
SCHEMA_VERSION = "triscopellm/post-route-c-frozen-semantic-minimal-real-batched-continuous-regression-analysis/v1"


def post_route_c_frozen_semantic_minimal_real_batched_continuous_regression_analysis(
    regression_dir: Path,
    output_dir: Path,
) -> dict[str, object]:
    output_dir.mkdir(parents=True, exist_ok=True)

    summary = load_json(regression_dir / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_summary.json")
    ...
    if (
        ...
        final_verdict = "Minimal real batched continuous regression validated"
        recommendation = "下一步进入冻结语义与 gate 不变前提下的最小 route_c batched continuous 稳定性复检"
        why = [
            "Recoverable formatting boundary stays valid on minimal real batched continuous route_c regression, including code-fence-like handoff.",
            "Normal and nonrecoverable guardrails remain intact with zero new false blocks and zero leaks.",
            "No new gate fallback, health anomaly, parser-source drift, or handoff contract violation appears relative to frozen 150/151/152/153 references.",
        ]
        ...
        why = [
            "Guardrail paths remain preserved, but recoverable boundary or batched-continuous consistency drift still exists.",
        ]
    else:
        final_verdict = "Not validated"
        recommendation = "下一步回到 recoverable-boundary 与真实连续 path handoff 收口，不准前进"
        why = [
            "Minimal real batched continuous regression reveals guardrail damage, leakage, or batched chain consistency failure.",
        ]
    ...
    analysis_summary = {
        ...
        "collateral_findings": {
            ...
            "real_execution_correct_but_real_batched_continuous_unstable": collateral[
    ...
    verdict = {
        ...
        "schema_version": SCHEMA_VERSION,
        "final_verdict": final_verdict,
        "single_verdict_policy": "one_of_minimal_real_batched_continuous_regression_validated__partially_validated__not_validated",
        "primary_basis": why,
    }
    ...
    recommendation_payload = {
        ...
        "do_not_do_yet": [
            ...
            "turn_154_into_large_real_batched_continuous_regression_stability_project",
        ],
        "regression_summary_path": str(
            (regression_dir / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_summary.json").resolve()
        ),
    }

    write_json(
        output_dir / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_analysis_summary.json",
        analysis_summary,
    )
    write_json(output_dir / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_verdict.json", verdict)
    write_json(
        output_dir / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_next_step_recommendation.json",
        recommendation_payload,
    )
    return {
        ...
        "output_paths": {
            "summary": str(
                (output_dir / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_analysis_summary.json").resolve()
            ),
            "verdict": str(
                (output_dir / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_verdict.json").resolve()
            ),
            "recommendation": str(
                (
                    output_dir
                    / "route_c_frozen_semantic_minimal_real_batched_continuous_regression_next_step_recommendation.json"
                ).resolve()
            ),
        },
    }
