"""Validation helpers for larger-split route C rerun artifacts."""
...
def validate_rerun_route_c_on_larger_split(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    plan_path = run_dir / "larger_split_route_c_plan.json"
    definition_path = run_dir / "larger_split_route_c_label_definition.json"
    readiness_path = run_dir / "larger_split_route_c_readiness_summary.json"
    dataset_path = run_dir / "larger_split_route_c_dataset.jsonl"
    summary_path = run_dir / "larger_split_route_c_summary.json"
    predictions_path = run_dir / "larger_split_route_c_logistic_predictions.jsonl"
    logistic_summary_path = run_dir / "larger_split_route_c_logistic_summary.json"
    run_summary_path = run_dir / "larger_split_route_c_run_summary.json"
    ...
    if compare_run_dir is not None:
        compare_summary = load_json(compare_run_dir / "larger_split_route_c_summary.json")
        compare_logistic = load_json(compare_run_dir / "larger_split_route_c_logistic_summary.json")
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM larger-split route C validation",
