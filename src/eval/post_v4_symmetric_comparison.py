"""Compare supervision routes after route B and route C reruns on labeled split v4."""
...
SCHEMA_VERSION = "triscopellm/post-v4-symmetric-comparison/v1"
...
def build_post_v4_symmetric_comparison(
    ...
    route_c_v3_dir: Path,
    route_c_v4_dir: Path,
    post_v3_comparison_dir: Path,
    post_v3_bootstrap_dir: Path,
    output_dir: Path,
) -> dict[str, Any]:
    ...
    prior_recommendation = load_json(post_v3_comparison_dir / "v3_symmetric_next_step_recommendation.json")
    v4_split_summary = load_json(post_v3_bootstrap_dir / "larger_labeled_split_v4_summary.json")
    ...
    comparison_rows = [
        ...
        },
        {
            ...
            "comparison_role": "v4_symmetric_candidate",
        },
        {
            "route_code": "C_old",
    ...
    progression_summary = {
        ...
        "v4_split_context": {
            "split_name": v4_split_summary["split_name"],
            "num_rows": v4_split_summary["num_rows"],
            "num_base_samples": v4_split_summary["num_base_samples"],
        },
        "progression": {
        ...
        "main_takeaway": "B and C now both exist on the same 50-row larger labeled split v4, so the next bottleneck again shifts from rerun asymmetry back to shared substrate growth.",
    }
    ...
    tradeoff_rows = [
        {
            "route_code": "B_v4",
            "route_name": "more_natural_supervision_proxy",
            "truth_anchor_level": route_b_v4_summary["label_naturalness_level"],
            "substrate_level": "larger_labeled_split_v4",
            "current_rows": route_b_v4_summary["num_rows"],
            "current_base_samples": route_b_v4_summary["num_base_samples"],
            "realism": "medium",
            "cost_to_expand_further": "medium",
            "next_ceiling": "shared_substrate_size",
            "why": "B now has symmetric substrate access at 50 rows, but it still uses the weaker more-natural proxy truth anchor.",
        },
        {
            "route_code": "C_v4",
            "route_name": "benchmark_truth_leaning_supervision_proxy",
            "truth_anchor_level": route_c_v4_summary["label_naturalness_level"],
            "substrate_level": "larger_labeled_split_v4",
            "current_rows": route_c_v4_summary["num_rows"],
            "current_base_samples": route_c_v4_summary["num_base_samples"],
            "realism": "medium_high",
            "cost_to_expand_further": "medium",
            "next_ceiling": "shared_substrate_size",
            "why": "C preserves the stronger truth-leaning semantics, but both B and C again share the same substrate ceiling at v4.",
        },
        {
            "route_code": "substrate_v5",
            "route_name": "larger_labeled_split_v5",
            "truth_anchor_level": "shared_substrate_enabler",
            "substrate_level": "next_bootstrap",
            "current_rows": v4_split_summary["num_rows"],
            "current_base_samples": v4_split_summary["num_base_samples"],
            "realism": "enabler",
            "cost_to_expand_further": "medium",
            "next_ceiling": "local_curated_split_growth",
            "why": "A larger shared substrate v5 would again unlock the next symmetric comparison step for both B and C beyond the current 50-row ceiling.",
        },
    ]
    ...
    summary = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "v4_symmetric_ready": True,
        "main_question_answer": "After route B and route C both exist on labeled split v4, the next highest-value step is to prepare larger_labeled_split_v5, because both routes now share the same 50-row substrate ceiling.",
        "why": [
            "Route B v4 and route C v4 now both run on the same 50-row shared substrate.",
    ...
    recommendation = {
        "summary_status": "PASS",
        "schema_version": SCHEMA_VERSION,
        "recommended_next_step": "prepare_larger_labeled_split_v5",
        "why_recommended": [
            "Route B and route C have both now been exercised on the same 50-row labeled split v4.",
            "Further route-specific reruns on the same v4 substrate would yield diminishing returns.",
            "A larger shared substrate v5 is the cleanest next enabler for both supervision routes.",
        ],
        "why_not_expand_B_first": [
            "B already has a true v4 rerun, so its main asymmetry problem has been removed at this substrate level.",
        ],
        "why_not_expand_C_first": [
            "C also already has a true v4 rerun, so another route-specific C-first step would be less informative than growing the shared substrate.",
        ],
        ...
        "minimum_success_standard": [
            "define larger split v5 contract",
            "materialize a larger shared labeled substrate beyond 50 rows",
            "prove route B / route C / fusion builders still attach cleanly",
        ],
    }

    write_json(output_dir / "v4_symmetric_comparison_summary.json", summary)
    write_csv(output_dir / "route_b_v4_vs_route_c_v4_comparison.csv", comparison_rows)
    write_json(output_dir / "supervision_progression_after_v4_rerun.json", progression_summary)
    write_csv(output_dir / "v4_tradeoff_matrix.csv", tradeoff_rows)
    write_json(output_dir / "v4_symmetric_next_step_recommendation.json", recommendation)
    (output_dir / "build.log").write_text(
        "\n".join(
            [
                "TriScope-LLM post-v4 symmetric comparison",
                f"route_b_v4_rows={route_b_v4_summary['num_rows']}",
                f"route_c_v4_rows={route_c_v4_summary['num_rows']}",
                "recommended_next_step=prepare_larger_labeled_split_v5",
            ]
        )
        + "\n",
    ...
    return {
        ...
        "output_paths": {
            "summary": str((output_dir / "v4_symmetric_comparison_summary.json").resolve()),
            "comparison_csv": str((output_dir / "route_b_v4_vs_route_c_v4_comparison.csv").resolve()),
            "progression": str((output_dir / "supervision_progression_after_v4_rerun.json").resolve()),
            "tradeoff_csv": str((output_dir / "v4_tradeoff_matrix.csv").resolve()),
            "recommendation": str((output_dir / "v4_symmetric_next_step_recommendation.json").resolve()),
