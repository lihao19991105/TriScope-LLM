"""Validation helpers for post-v5 symmetric comparison artifacts."""
...
def validate_post_v5_symmetric_comparison(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    summary_path = run_dir / "v5_symmetric_comparison_summary.json"
    comparison_csv_path = run_dir / "route_b_v5_vs_route_c_v5_comparison.csv"
    progression_path = run_dir / "supervision_progression_after_v5_rerun.json"
    tradeoff_csv_path = run_dir / "v5_tradeoff_matrix.csv"
    recommendation_path = run_dir / "v5_symmetric_next_step_recommendation.json"
    ...
    acceptance = {
        ...
        "field_checks": {
            ...
            "comparison_csv_has_rows": count_csv_rows(comparison_csv_path) >= 12,
    ...
    if compare_run_dir is not None:
        compare_recommendation = load_json(compare_run_dir / "v5_symmetric_next_step_recommendation.json")
        compare_progression = load_json(compare_run_dir / "supervision_progression_after_v5_rerun.json")
        repeatability_summary = {
            ...
            "comparisons": [
                ...
                {
                    "field": "route_b_v5_rows",
                    "reference_value": progression.get("progression", {}).get("route_b_v5_rows"),
                    "candidate_value": compare_progression.get("progression", {}).get("route_b_v5_rows"),
                    "matches": progression.get("progression", {}).get("route_b_v5_rows")
                    == compare_progression.get("progression", {}).get("route_b_v5_rows"),
                },
                {
                    "field": "route_c_v5_rows",
                    "reference_value": progression.get("progression", {}).get("route_c_v5_rows"),
                    "candidate_value": compare_progression.get("progression", {}).get("route_c_v5_rows"),
                    "matches": progression.get("progression", {}).get("route_c_v5_rows")
                    == compare_progression.get("progression", {}).get("route_c_v5_rows"),
                },
            ],
        }
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM post-v5 symmetric comparison validation",
