"""Validation helpers for next-axis-after-v8 matrix bootstrap."""
...
def validate_next_axis_after_v8_matrix_bootstrap(
    run_dir: Path,
    compare_run_dir: Path,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    summary = load_json(run_dir / "next_axis_after_v8_bootstrap_summary.json")
    definition = load_json(run_dir / "next_axis_after_v8_matrix_definition.json")
    compare_summary = load_json(compare_run_dir / "next_axis_after_v8_bootstrap_summary.json")
