"""Dry-run the richer next real-experiment matrix v2."""
...
SCHEMA_VERSION = "triscopellm/next-real-experiment-matrix-dry-run/v1"
...
def build_next_real_experiment_matrix_dry_run(
    ...
    if matrix_definition.get("summary_status") != "PASS":
        raise ValueError("Next matrix definition must be PASS before dry-run.")
    if matrix_bootstrap_summary.get("summary_status") != "PASS":
        raise ValueError("Next matrix bootstrap summary must be PASS before dry-run.")
    ...
    for path in [*required_matrix_inputs, *required_v6_inputs]:
        if not path.is_file():
            raise ValueError(f"Next matrix dry-run input not found: `{path}`.")

    materialized_execution_dir = output_dir / "materialized_next_matrix_inputs"
    materialized_execution_dir.mkdir(parents=True, exist_ok=True)
    for src in [*required_matrix_inputs, *required_v6_inputs]:
    ...
    cells = [
        {
            "cell_id": "dataset0_model0_routes_b_c_fusion",
            "dataset_name": matrix_definition["datasets"][0],
            "model_name": matrix_definition["models"][0],
            "enabled_routes": ["route_b", "route_c", "fusion_summary"],
            "cell_role": "full_route_bundle",
        },
        {
    ...
    plan = {
        ...
        "dry_run_mode": "richer_matrix_level_realistic_simulation",
        "seed": seed,
    ...
    contract = {
        ...
        "cells": cells,
        "route_role_map": {
            "route_b": "full more-natural route inside the full richer matrix cell",
            "route_c": "full benchmark-truth-leaning route inside the full richer matrix cell",
            "fusion_summary": "integrated summary across full route B and route C coverage",
    ...
    readiness = {
        ...
        "notes": [
            "The richer matrix keeps the same dataset/model substrate but promotes route coverage into explicit full and ablation cells.",
            "route_b_only_ablation and route_c_only_ablation are first-class matrix cells in this contract, not just comments.",
        ],
    }
    write_json(output_dir / "next_matrix_dry_run_plan.json", plan)
    write_json(output_dir / "next_matrix_execution_contract.json", contract)
    write_json(output_dir / "next_matrix_readiness_summary.json", readiness)
    ...
    module_status = {
        ...
        "modules": {
            ...
            "route_b": {"status": "PASS", "expected_rows": reasoning_budget, "mapping_key": "sample_id"},
            "route_c": {"status": "PASS", "expected_rows": labeled_budget, "mapping_key": "base_sample_id+contract_variant"},
            "fusion_summary": {"status": "PASS", "mode": "richer_matrix_full_cell_summary"},
            "route_b_only_ablation": {"status": "PASS", "expected_rows": reasoning_budget, "ablation_target": "route_b_isolated_signal"},
            "route_c_only_ablation": {"status": "PASS", "expected_rows": labeled_budget, "ablation_target": "route_c_isolated_signal"},
        },
    }
    cell_status = {
        "summary_status": "PASS",
        ...
        "cells": [
            {
                ...
                "dataset_name": cells[0]["dataset_name"],
                "model_name": cells[0]["model_name"],
                "enabled_routes": cells[0]["enabled_routes"],
                "status": "PASS",
                "blocked_reason": "",
            },
            {
                "cell_id": cells[1]["cell_id"],
                "dataset_name": cells[1]["dataset_name"],
                "model_name": cells[1]["model_name"],
                "enabled_routes": cells[1]["enabled_routes"],
                "status": "PASS",
                "blocked_reason": "",
            },
            {
                "cell_id": cells[2]["cell_id"],
                "dataset_name": cells[2]["dataset_name"],
                "model_name": cells[2]["model_name"],
                "enabled_routes": cells[2]["enabled_routes"],
                "status": "PASS",
                "blocked_reason": "",
            },
        ],
    ...
    registry = {
        ...
        "matrix_name": matrix_bootstrap_summary["matrix_name"],
        "artifacts": {
            "plan": str((output_dir / "next_matrix_dry_run_plan.json").resolve()),
            "contract": str((output_dir / "next_matrix_execution_contract.json").resolve()),
            "module_status": str((output_dir / "next_matrix_module_status.json").resolve()),
            "cell_status": str((output_dir / "next_matrix_cell_status.json").resolve()),
        },
    }
    preview_rows = [
        {
            ...
            "route_b_mapped": True,
            "route_c_mapped": True,
            "fusion_summary_mapped": True,
            "route_b_only_ablation_mapped": True,
            "route_c_only_ablation_mapped": True,
        }
    ...
    summary = {
        ...
        "passed_routes": matrix_definition["routes"],
        "passed_ablation_cells": [cells[1]["cell_id"], cells[2]["cell_id"]],
    }
    write_json(output_dir / "next_matrix_module_status.json", module_status)
    write_json(output_dir / "next_matrix_cell_status.json", cell_status)
    write_json(output_dir / "next_matrix_dry_run_registry.json", registry)
    write_json(output_dir / "next_matrix_dry_run_summary.json", summary)
    write_jsonl(output_dir / "next_matrix_dry_run_preview.jsonl", preview_rows)
    (output_dir / "next_matrix_dry_run.log").write_text(
        "\n".join(
            [
                "TriScope-LLM next real-experiment matrix v2 dry-run",
    ...
    return {
        ...
        "output_paths": {
            "plan": str((output_dir / "next_matrix_dry_run_plan.json").resolve()),
            "contract": str((output_dir / "next_matrix_execution_contract.json").resolve()),
            "readiness": str((output_dir / "next_matrix_readiness_summary.json").resolve()),
            "summary": str((output_dir / "next_matrix_dry_run_summary.json").resolve()),
            "registry": str((output_dir / "next_matrix_dry_run_registry.json").resolve()),
        },
    }
