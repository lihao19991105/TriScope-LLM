...
def insert_trigger(prompt: str, trigger_text: str, position: str = TRIGGER_INSERT_POSITION) -> str:
    ...
    if position == "instruction_suffix":
        return f"{prompt.rstrip()} {trigger_text}"
...
def validate_labeled_rows(rows: list[dict[str, Any]], source_count: int | None = None) -> dict[str, Any]:
    ...
    return {
        "summary_status": "PASS" if not row_errors and not invalid_pairs and (expected_rows is None or len(rows) == expected_rows) else "FAIL",
        ...
        "pairing_valid": not invalid_pairs and (expected_rows is None or len(rows) == expected_rows),
    }
