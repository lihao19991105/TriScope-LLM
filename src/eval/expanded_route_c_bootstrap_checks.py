"""Validation helpers for expanded route-C bootstrap artifacts."""
...
def validate_expanded_route_c_bootstrap(
    run_dir: Path,
    compare_run_dir: Path | None,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    selection_path = run_dir / "expanded_route_c_selection.json"
    definition_path = run_dir / "expanded_route_c_label_definition.json"
    readiness_path = run_dir / "expanded_route_c_readiness_summary.json"
    dataset_path = run_dir / "expanded_benchmark_truth_leaning_dataset.jsonl"
    summary_path = run_dir / "expanded_benchmark_truth_leaning_summary.json"
    predictions_path = run_dir / "expanded_benchmark_truth_leaning_logistic_predictions.jsonl"
    logistic_summary_path = run_dir / "expanded_benchmark_truth_leaning_logistic_summary.json"
    run_summary_path = run_dir / "expanded_route_c_run_summary.json"
    ...
    if compare_run_dir is not None:
        compare_summary = load_json(compare_run_dir / "expanded_benchmark_truth_leaning_summary.json")
        compare_logistic = load_json(compare_run_dir / "expanded_benchmark_truth_leaning_logistic_summary.json")
    ...
    (output_dir / "repeat_check.log").write_text(
        "\n".join(
            [
                "TriScope-LLM expanded route C validation",
