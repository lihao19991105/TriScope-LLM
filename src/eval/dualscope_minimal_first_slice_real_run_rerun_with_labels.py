...
def _metric_counts(joined_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    flagged_positive = sum(
        1 for row in joined_rows if row.get("detection_label") == 1 and row.get("final_decision_flag") is True
    )
    flagged_negative = sum(
        1 for row in joined_rows if row.get("detection_label") == 0 and row.get("final_decision_flag") is True
    )
    return {
...
def build_labeled_real_run_rerun(
    ...
    manifest = _read_optional_json(label_output_dir / "dualscope_first_slice_clean_poisoned_pair_manifest.json")
    label_rows_path = Path(manifest.get("output_file", "data/stanford_alpaca/first_slice/alpaca_first_slice_labeled_pairs.jsonl"))
    ...
    condition_level_predictions_ready = all(
        row.get("condition") in {"clean", "poisoned_triggered"}
        and row.get("stage3_prediction_scope") == "condition_row_level"
    ...
    write_jsonl(output_dir / "dualscope_minimal_first_slice_real_run_rerun_with_labels_joined_rows.jsonl", joined_rows)
    write_jsonl(
    ...
    markdown(
        ...
        [
            ...
            "- Stage 3 prediction scope: `source_example_level`",
