"""Repair/compression package for JBB small-slice response generation."""
...
TASK_ID = "dualscope-jbb-small-slice-response-generation-repair"
SOURCE_TASK_ID = "dualscope-jbb-small-slice-response-generation"
SCHEMA_VERSION = "dualscope/jbb-small-slice-response-generation-repair/v1"
VALIDATED = "JBB small-slice response generation repair validated"
PARTIAL = "Partially validated"
NOT_VALIDATED = "Not validated"

DEFAULT_RESPONSE_DIR = Path("outputs/dualscope_jbb_small_slice_response_generation/default")
DEFAULT_OUTPUT_DIR = Path("outputs/dualscope_jbb_small_slice_response_generation_repair/default")
DEFAULT_REGISTRY_PATH = Path(".reports/dualscope_task_verdicts/dualscope-jbb-small-slice-response-generation-repair.json")
...
def _blocker_route(blocker_type: str, blockers: list[str]) -> tuple[str, str]:
    ...
    if "missing" in text:
        return "dualscope-jbb-small-slice-response-input-artifact-repair", "missing_input_or_dependency"
    if "oom" in text or "memory" in text:
        return "dualscope-jbb-small-slice-response-generation-blocker-closure", "gpu_memory_blocker"
    if "cuda" in text or "nvidia" in text or "driver" in text:
        return "dualscope-jbb-small-slice-response-generation-blocker-closure", "cuda_visibility_blocker"
    if "safety_filter" in text:
        return "dualscope-jbb-small-slice-response-generation-blocker-closure", "output_safety_filter_blocker"
    if "logprob" in text:
        return "dualscope-jbb-small-slice-metric-computation", "without_logprobs_fallback_ready"
    return "dualscope-jbb-small-slice-response-generation-blocker-closure", "runtime_blocker"
...
def _availability_matrix(summary: dict[str, Any], row_audit: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "clean_utility": {
            "status": "blocked",
            "reason": "JBB harmful-behavior small slice has no clean utility reference set",
        },
        "full_matrix": {
            "status": "not_run",
            "reason": "bounded JBB small-slice only",
        },
    }
...
def build_jbb_small_slice_response_generation_repair(
    *,
    ...
    summary_path = response_dir / "jbb_small_slice_response_generation_summary.json"
    blockers_path = response_dir / "jbb_small_slice_response_generation_blockers.json"
    verdict_path = response_dir / "jbb_small_slice_response_generation_verdict.json"
    rows_path = response_dir / "jbb_small_slice_responses.jsonl"
    ...
    if missing_inputs:
        ...
        next_step = "dualscope-jbb-small-slice-response-generation-blocker-closure"
        ...
        next_step = "dualscope-jbb-small-slice-metric-computation"
    ...
    write_json(output_dir / "jbb_small_slice_response_generation_repair_summary.json", summary)
    write_json(output_dir / "jbb_small_slice_response_generation_repair_availability_matrix.json", availability)
    write_json(output_dir / "jbb_small_slice_response_generation_repair_blocker_compression.json", blocker_compression)
    write_json(output_dir / "jbb_small_slice_response_generation_repair_row_audit.json", row_audit)
    write_jsonl(output_dir / "jbb_small_slice_response_generation_repair_compact_rows.jsonl", compact_rows)
    write_json(output_dir / "jbb_small_slice_response_generation_repair_verdict.json", verdict)
    write_text(output_dir / "jbb_small_slice_response_generation_repair_report.md", _report(summary))
