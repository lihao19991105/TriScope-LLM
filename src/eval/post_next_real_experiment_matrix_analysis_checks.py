"""Validation helpers for post next real-experiment matrix analysis."""
...
def validate_post_next_real_experiment_matrix_analysis(
    run_dir: Path,
    compare_run_dir: Path,
    output_dir: Path,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    analysis = load_json(run_dir / "next_matrix_analysis_summary.json")
    recommendation = load_json(run_dir / "next_matrix_next_step_recommendation.json")
    compare_analysis = load_json(compare_run_dir / "next_matrix_analysis_summary.json")
