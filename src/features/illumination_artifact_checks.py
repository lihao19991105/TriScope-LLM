"""Artifact acceptance and repeatability checks for illumination runs."""
...
REQUIRED_ARTIFACTS = {
    "raw_results": {
        ...
        "purpose": "Original prompt-level probing results for illumination analysis and reprocessing.",
    },
    "config_snapshot": {
        "relative_path": "config_snapshot.json",
        "purpose": "Resolved model, illumination, and runtime configuration used by the probe run.",
    },
    "summary": {
        "relative_path": "summary.json",
        "purpose": "Minimal probing summary with target-behavior counts and budget realization.",
    },
    "prompt_level_features": {
        "relative_path": "features/prompt_level_features.jsonl",
        "purpose": "Prompt-level machine-readable illumination feature rows for downstream fusion alignment.",
    },
    "aggregated_features": {
        "relative_path": "features/illumination_features.json",
        "purpose": "Run-level aggregated illumination feature payload for fusion and analysis.",
    },
    "feature_summary": {
        "relative_path": "features/feature_summary.json",
        "purpose": "Compact summary of extracted illumination features for smoke checks and README references.",
    },
}
...
def validate_illumination_run_artifacts(run_dir: Path) -> dict[str, Any]:
    ...
    prompt_feature_rows = load_jsonl_rows(
        run_dir / REQUIRED_ARTIFACTS["prompt_level_features"]["relative_path"]
    )
    ...
    field_checks = {
        ...
        "prompt_features_non_empty": len(prompt_feature_rows) > 0,
    ...
    required_prompt_fields = {"run_id", "probe_id", "sample_id", "source_sample_ids", "is_target_behavior"}
    required_run_fields = {
        "num_prompts",
        "num_target_behavior",
    ...
    first_prompt = prompt_feature_rows[0] if prompt_feature_rows else {}
    run_features = aggregated_payload.get("run_features", {})
    field_checks["prompt_feature_required_fields"] = required_prompt_fields.issubset(first_prompt)
    ...
    return {
        ...
        "artifact_counts": {
            ...
            "prompt_level_features": len(prompt_feature_rows),
        },
        "run_feature_snapshot": {
            "run_id": run_features.get("run_id"),
            "model_profile": run_features.get("model_profile"),
            "illumination_profile": run_features.get("illumination_profile"),
            "num_prompts": run_features.get("num_prompts"),
            "num_target_behavior": run_features.get("num_target_behavior"),
            "target_behavior_rate": run_features.get("target_behavior_rate"),
            "realized_budget_ratio": run_features.get("realized_budget_ratio"),
        },
    }


def compare_illumination_runs(reference_run_dir: Path, candidate_run_dir: Path) -> dict[str, Any]:
    reference = validate_illumination_run_artifacts(reference_run_dir)
    candidate = validate_illumination_run_artifacts(candidate_run_dir)
    ...
    return {
        "summary_status": "PASS" if reference["summary_status"] == "PASS" and candidate["summary_status"] == "PASS" and all_match else "FAIL",
        "reference_run_dir": str(reference_run_dir.resolve()),
        "candidate_run_dir": str(candidate_run_dir.resolve()),
        "reference_acceptance": reference,
        "candidate_acceptance": candidate,
