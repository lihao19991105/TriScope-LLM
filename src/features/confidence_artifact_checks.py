"""Artifact acceptance and repeatability checks for confidence runs."""
...
REQUIRED_ARTIFACTS = {
    "raw_results": {
        ...
        "purpose": "Original token-level confidence probe results with per-step probabilities and entropies.",
    },
    "config_snapshot": {
        "relative_path": "config_snapshot.json",
        "purpose": "Resolved model, confidence, and runtime configuration used by the confidence run.",
    },
    "summary": {
        "relative_path": "summary.json",
        "purpose": "Minimal confidence summary with generated-token and entropy statistics.",
    },
    "sample_level_features": {
        "relative_path": "features/confidence_prompt_level_features.jsonl",
        "purpose": "Sample-level machine-readable confidence feature rows for downstream fusion alignment.",
    },
    "aggregated_features": {
        "relative_path": "features/confidence_features.json",
        "purpose": "Run-level aggregated confidence feature payload for fusion and analysis.",
    },
    "feature_summary": {
        "relative_path": "features/feature_summary.json",
        "purpose": "Compact summary of extracted confidence features for smoke checks and README references.",
    },
}
...
def validate_confidence_run_artifacts(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "run_feature_snapshot": {
            ...
            "confidence_profile": run_features.get("confidence_profile"),
            "num_samples": run_features.get("num_samples"),
            "target_behavior_rate": run_features.get("target_behavior_rate"),
            "mean_chosen_token_prob_mean": run_features.get("mean_chosen_token_prob_mean"),
            "mean_entropy_mean": run_features.get("mean_entropy_mean"),
            "high_confidence_fraction_mean": run_features.get("high_confidence_fraction_mean"),
            "entropy_collapse_score_mean": run_features.get("entropy_collapse_score_mean"),
        },
    }


def compare_confidence_runs(reference_run_dir: Path, candidate_run_dir: Path) -> dict[str, Any]:
    reference = validate_confidence_run_artifacts(reference_run_dir)
    candidate = validate_confidence_run_artifacts(candidate_run_dir)
